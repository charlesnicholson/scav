...
std::string_view bytes_view(scav_byte const *bytes, uint32_t len) {
  // char may alias any object representation, which is exactly what this cast
  // relies on; the other direction would not be safe.
...
void finalize(Interner const &in, StringPool &pool, StringRemap &out) {
  ...
  stable_sort_by(order, [staged, &unique](uint32_t a, uint32_t b) {
    return compare_bytes(staged + unique[a].off,
                         unique[a].len,
                         staged + unique[b].off,
                         unique[b].len) < 0;
  });
