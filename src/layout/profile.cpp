...
constexpr Profile READABLE{
  ...
  // History, DeepHistory.
  .kind_min_w = { 40 * PT, 14 * PT, 14 * PT, 20 * PT, 12 * PT, 60 * PT, 60 * PT,
                  16 * PT, 16 * PT },
  .kind_min_h = { 24 * PT, 14 * PT, 14 * PT, 20 * PT, 12 * PT, 4 * PT, 4 * PT,
                  16 * PT, 16 * PT },
  .dar_num = 16,
  .dar_den = 10,
  .trybox = 1,
...
constexpr Profile COMPACT{
  ...
  .line_height_k_num = 6,
  .line_height_k_den = 5,
  .kind_min_w = { 32 * PT, 12 * PT, 12 * PT, 16 * PT, 10 * PT, 48 * PT, 48 * PT,
                  14 * PT, 14 * PT },
  .kind_min_h = { 18 * PT, 12 * PT, 12 * PT, 16 * PT, 10 * PT, 3 * PT, 3 * PT,
                  14 * PT, 14 * PT },
  .dar_num = 4,
  .dar_den = 3,
  .trybox = 1,
...
bool profile_validate(Profile const &p) {
  ...
  bool ok{ in_range(p.profile_id, 0, I32_MAX) &&
           ...
           in_range(p.pad, 0, SPACE_MAX) &&
           in_range(p.font_size_grid, 1, SPACE_MAX) &&
           in_range(p.line_height_k_num, 1, 1024) &&
           in_range(p.line_height_k_den, 1, 1024) &&
           in_range(p.dar_num, 1, 1024) && in_range(p.dar_den, 1, 1024) &&
           in_range(p.trybox, 0, 1) && in_range(p.sm_tiebreak, 0, 1) &&
           in_range(p.w_bends, 0, 1024) && in_range(p.w_corridor, 0, 1024) &&
           in_range(p.w_crossings, 0, 1024) && in_range(p.w_excess_len, 0, 1024) &&
           in_range(p.w_adjacency, 0, 1024) && in_range(p.w_label, 0, 1024) &&
           in_range(p.w_aspect, 0, 1024) && in_range(p.w_area, 0, 1024) &&
           in_range(p.portfolio_k, 1, 64) && in_range(p.sweep_count, 0, 1024) &&
           in_range(p.congestion_iterations, 0, 1024) &&
           in_range(p.ripup_cap, 0, 1024) &&
           in_range(p.spacing_inflation_cap, 0, 1024) &&
           in_range(p.spacing_inflation_increment, 0, SPACE_MAX) &&
           in_range(p.print_columns, static_cast<int32_t>(PRINT_COLUMNS_MIN),
