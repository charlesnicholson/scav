...
The first run takes a while: [envy](https://github.com/envy-package-manager/envy)
bootstraps itself, then provisions cmake, ninja, doctest, python 3.14 and the
pinned clang-format and clang-tidy into `out/.envy`. After that it is a cache
hit. On macOS that first run compiles clang, because LLVM publishes no darwin
prebuilt; point `ENVY_CACHE_ROOT` at a cache outside the tree and `rm -rf out`
will not ask for it twice:

```
export ENVY_CACHE_ROOT="$HOME/Library/Caches/envy"   # macOS
export ENVY_CACHE_ROOT="$HOME/.cache/envy"           # Linux
```
...
**Nothing comes from the system but the compiler.** cmake, ninja, doctest,
python, clang-format and clang-tidy are envy's — CI's Linux container ships its
own copies and scav ignores them, which `func.provisioning` enforces. Both
gates run the version `envy.lua` pins, so a finding reads the same everywhere.
...
| `bin/` | envy's product launchers, checked in and deployed by `envy sync`. Each one bootstraps envy and its package cache on a machine that has neither |
