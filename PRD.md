...
Two consequences of the rules above, stated because each looks like a defect until it is read as canonical form doing its job. **The `chart` block always breaks**, whatever the budget says: a document is a file, and a one-line file makes every edit a whole-file diff. **A blank line between two statements does not survive**, because no row records one — the model holds statements and trivia, and a gap between them is neither. A blank line *after a comment* does survive, since `CommentPos::OwnLine` is exactly that fact. Restoring the rest would be one bit on `Statement`; nothing yet needs it, and adding it later costs a reprint of the corpus and nothing else.
