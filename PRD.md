...
**The counterfactual is how a finding is sorted.** `scav dump --layout` prints the row and every pin a drawing rests on as `rests on --portfolio-row R --rank S:R --cut T:L --reverse T:L --face T:L:E:F`, and `--no-search` with those flags lays that drawing out again exactly. An edited copy scores the drawing a reader wants on the shipped objective: cheaper than what shipped means the search lacks a move, dearer means the objective is wrong.
...
2. **`[OWED]` Alignment in both axes.** Across: Brandes–Köpf aligns an edge's attachment points rather than its nodes' centres, so a port lines up with what it connects to and a transition runs straight through every frame on its path. Along: a node's place in its column lines up with the ends of its edges within the column, as `Reached` under `Ready` and `Empty` under `On` want.
