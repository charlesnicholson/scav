...
src/core/load/         the load session, document-name resolution, the filesystem
...
- include cycles and unresolvable include paths — the **load session's**, not `validate_chart`'s. Neither is representable in a finished chart, because `finish` refuses to produce one; checking for them afterwards would be checking for a state that cannot exist. Their diagnostics are therefore document-local, since they fire precisely when no chart does
