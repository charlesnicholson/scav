...
w_lbl  * label_overlaps                              -- three components, below
...
**P7d — labels landed; the inflation and the bench still [OWED].** `PathBox` strip placement by Kakoulis & Tollis matching is built (§11.9) and called from phase 3 once the routes are final and nudged. Still owed under this name: the degenerate-enclosure inflation and its diagnostic, and the bench harness over the two registered routers.
