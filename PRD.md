...
|---|---|
| PB bootstrap, P0 language, P1 model spine, P2 loader, P3 printer and CLI, P4 space tables, P5a metrics + `DrawList` + reference builder, P5b SVG + baseline harness, P5c ABI JSON + Python binding, P6 real layout, P7a router boundary, P7b orthogonal router, P7c nudging, P7d label placement, inflation, and the bench, P7e the combinatorial stage, the attachment, and the element suite, P8 the thread shim, the sharded phases, the seeded structural hash, the determinism class, and `selftest`, P9a the docs commit, `crowd.scav`, the hierarchy-descent scorer, Tier 2 in ems, and profile v5 | **the incumbent comparison**, rescheduled to P9's exit because a single-candidate render measures the missing search (§11.12) · **routing between two concurrent submachines of one state** (§11.8), which no corpus chart exercises and the element suite pins · rip-up-and-reroute for the boxes no strip fits (§11.9) · **P9b–P9e search and calibration** (§11.10): P9b the surrogate and the phase-1 hoist · P9c the two-level portfolio and the published latency form · P9d local search · P9e the fitted weights and the side-by-side · P10 viewer · P11 wasm · P12 editor |
...
**Four search fields are validated and read by nothing.** `portfolio_k`, `portfolio_m`, `congestion_iterations` and `ripup_cap` are bounds-checked at load and hashed into the inputs digest (§6), so changing one moves a golden's inputs column without moving a coordinate. `portfolio_m` landed at P9a on the profile bump the em already forced, and both portfolio fields become live at P9c; the other two belong to stages §11.5 and §11.9 hold as design and nothing has scheduled. `sweep_count` and the two spacing-inflation fields are what the tree reads of that group (§11.15).
...
**[OWED in full: nothing here is built.]** `portfolio_k` and `portfolio_m` are validated and read by nothing (§0, §11.15), and `sweep_count` is read by phase 1's crossing-minimisation sweeps (§11.3) and by nothing else. Everything below is design owned by this section and scheduled as §17's P9b–P9e; what P9a landed of it is the two-level shape itself, written here before anything was built against it.
...
| search | `portfolio_k`, `portfolio_m`, `sweep_count`, `congestion_iterations`, `ripup_cap`, `spacing_inflation_cap` and `spacing_inflation_increment`. **Three of the seven are read today**: `sweep_count`, which phase 1 spends on crossing-minimisation sweeps (§11.3), and the two spacing-inflation fields, which drive §11.6's retry. `portfolio_k`, `portfolio_m`, `congestion_iterations` and `ripup_cap` are validated and hashed and consumed by nothing until §11.10's portfolio (P9c) and the stages §11.5 and §11.9 hold as design. **`portfolio_m`**, for §11.10's chart-level portfolio, landed at P9a in `[1, 64]` and ships at 1, riding the `profile_version` bump the em already forced rather than paying for a second output-format change. `sweep_count` bounds every fixed-count improvement loop, phase 1's sweeps included — one knob because they are one question, splittable at **P9e** if calibration wants different numbers |
| format | `print_columns` — the canonical printer's line-break budget (§15) |
| id | `profile_id`, `profile_version` — 5 on both shipped profiles, `portfolio_m` being the field that last moved it |
