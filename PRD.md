...
|---|---|
| PB bootstrap, P0 language, P1 model spine, P2 loader, P3 printer and CLI, P4 space tables, P5a metrics + `DrawList` + reference builder, P5b SVG + baseline harness, P5c ABI JSON + Python binding, P6 real layout, P7a router boundary, P7b orthogonal router, P7c nudging, P7d label placement, inflation, and the bench, P7e the combinatorial stage, the attachment, and the element suite, P8 the thread shim, the sharded phases, the seeded structural hash, the determinism class, and `selftest`, P9a the docs commit, `crowd.scav`, the hierarchy-descent scorer, Tier 2 in ems, and profile v5, P9b Level 2 of the portfolio, the phase-1 hoist, DAR as a capability, and profile v6, P9c the spot list, whitespace elimination always, and compaction as Level 2's bit 1 | **the incumbent comparison**, rescheduled to P9d's exit because a render measures the search behind it, and at P9b that is one of two levels with the weights still unfitted (§11.12) · **routing between two concurrent submachines of one state** (§11.8), which no corpus chart exercises and the element suite pins · rip-up-and-reroute for the boxes no strip fits (§11.9) · **P9d's six label and attachment defects** (§11.9.3), found on the page 2026-09-09 and **ahead of both P9d and P9e**: four invariants the objective could not see, the placer's own endpoint exemption, the label charge the fold discards, the mis-tiered lane rules, and the boundary-node corners · **P9d–P9e calibration and the fine-grained level** (§11.10, §11.4), the order **set 2026-09-08** so the level that chooses between whole charts was built first and the fine-grained one is built last: P9d the fitted weights and the side-by-side · P9e Level 1 and local search, **gated on a measurement taken there**· P10 viewer · P11 wasm · P12 editor |
...
**P9d — the objective un-blinded first, then the weights fitted, then the side-by-side that has waited since P6.** **Reordered 2026-09-09 on a measurement.** The page was read against the shipped pick of every corpus chart before any weight was touched, and it returned six absolute defects of which five neither `tools/audit.py` nor `Cost` could count (§11.9.3). Landed: a row of §11.10's table can be pinned so calibration can render a candidate the objective would never pick; the layout dump carries the cost vector so the number that ranked a candidate travels beside the defects a reader sees in it; `tools/candidates.py` renders and audits the whole table and reports the per-class floor and the pick's regret against it; three audit classes and one three-site exemption; and the drawn corner no seat may sit on. Reader-visible defects read **227 shipped, 187 in the best candidate the table holds, 157 as the per-class floor** — against 100/69/62 while the instrument was blind, and **200 of the 227 are labels**. A weight fitted against an objective blind to four of its own violations would fit the wrong thing, so the fit begins after §11.9.3's owed work rather than before it.
