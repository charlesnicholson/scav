...
| | chosen by | searched |
|---|---|---|
| which rank a state is in | longest path after cycle breaking (§11.3) | no |
...
| where a route is seated | per box, and until 2026-09-19 never across two (§11.10a) | no |
| the whole-chart knobs | `argmin(Cost, row)` over a fixed table | **the only search there is** |

**So `Cost` ranks pre-declared drawings; it never shapes one.** "Minimise bends", "minimise area", "minimise crossings" are true of the objective and false of the pipeline: the terms choose between the eight arrangements the table names and have no way to straighten a route or move a state. A weight cannot fix an indirect route for the same reason a scoreboard cannot fix a pass. **What closes that gap is §11.10a's placement dimension and §11.10's own bounded moves, neither built**, and the measurements this section now carries say which are worth building.

**What *is* settled is the absolute layer.** Every Tier-0-shaped count is zero, and so are label slices, unanchored boxes, attachments on a drawn corner, labels outside their enclosing state, arrowheads through a border, and labels over another route (§11.12). A drawing scav produces today is *correct* and *crowded* — 37 crowded lane pairs of 59 remaining defects — and the crowding is separation, not search.
