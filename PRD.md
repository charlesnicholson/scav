...
Bundles then enter in the key's order and each takes the position contradicting the fewest votes, **the last of the positions that tie** — so a bundle no vote separates from those already placed lands after them, a lane with no incidences comes out in the key order it went in with, and a lane whose votes are consistent comes out a linear extension of them. Segment order is a total integral key and placement is the same **integer offsets `k*gap`** as before; the pseudo-direction pass the stage was first specified with is unnecessary, because ordering both members from the lane's own low end already measures them from one side (above).
...
**The element suite** (§5) is the durable half. Nine charts under `test_data/charts/gauntlet/`, one shape each, held to reader-visible properties at both shipped profiles — Tier 0, axis alignment, no leg reversed, an end on an inscribed glyph at a face midpoint, no arrowhead inked over another route's end, a fork's bar used along its length, a fan-in with none of its arrivals hidden inside another. It paid for itself on the first run: **§11.8's own case does not route** (two edges through a box, two routes doubling back, and no corpus chart carrying the shape at all), a mark with more than two incident transitions doubled a seat, and a two-state cycle sends one route the long way round the frame at the compact profile. The middle one was misread as §11.3's owed port sides and is not: `marks.scav` has no compound state and so no port, the seat was §11.5's, and the reface below closed it — the property now holds on every chart at both profiles with no carve-out. Each of the other two is carved out with its count pinned beside it and its section named, so the next change to either is a number that moved.
*Exit, met:* the two stages P7 named and did not build are built, every corpus golden rebased, and the element suite green with two open shapes counted rather than excused.
