...
**Tier 0 — forbidden, not priced.** Edge through a state box, submachine box, or placed box; box-box overlap. Structurally impossible via the obstacle set. The predicate survives as a net for three cases: the straight-line **surrogate** during search; **degenerate enclosure**, a net whose ends the obstacles seal apart even after §11.5's re-seat; and a marked violation with a stable code when the retries run out. Never a silent overlap.
