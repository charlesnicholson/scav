...
Reconciled against the tree on 2026-09-04. Phase names are §17's.

| Built and green | Owed, with an owner section |
|---|---|
| PB bootstrap, P0 language, P1 model spine, P2 loader, P3 printer and CLI, P4 space tables, P5a metrics + `DrawList` + reference builder, P5b SVG + baseline harness, P5c ABI JSON + Python binding, P6 real layout, P7a router boundary, P7b orthogonal router | **the blind review's recorded result** — it ran at P7b and only the compaction finding was written down (§11.12) · P7c separator channels, congestion, nudging (§11.5) · P7d label placement and the bench (§11.9) · P8 determinism infrastructure and `selftest` (§6) · P9 search and calibration (§11.10) · P10 viewer · P11 wasm · P12 editor |
...
Order-preserving and gap-avoiding are one constraint: restricting placement to four positions relative to the predecessor (directly right; right on the current row level; next subrow; next row) is exactly what makes local whitespace elimination always possible. **Compaction and whitespace elimination are the two steps not taken**: both reclaim leftover space, and both want blind review to judge before they ship.
...
**Nudging, as built, is the room-only half of the stage below.** A lane is a run of collinear overlapping segments in one frame, keyed `(axis, coordinate)` and swept into groups by extent. Its members are ordered by where each net was *before* it reached the lane, taken from whichever end is lower along the lane's own axis so every member is measured from the same side — a net arriving from above stays above rather than swapping and paying a crossing for it. Ordering by both ends reads worse, 145 corpus crossings against 129, because a net that changes side has no consistent answer and ends up placed by its tie-break.

**Only a segment with a neighbour at each end moves.** An end segment is anchored on a box border and sliding it along that face is a different move; 86% of the corpus's shared length is between two interior segments, so the anchored case buys little and risks detaching a route. **And a displacement is taken only when it is known good**: the segment and the two legs it drags may not enter an obstacle grown by `clear` that they were not already in, may not leave the region, and may not collapse a leg to nothing — which the arrowhead reads its direction off. A lane with no room keeps its members stacked, because a diagram that scores well and overlaps a box is worth less than one that scores badly and does not. The window either side is not symmetric — a box one side and open space the other is the ordinary case — so the bundle sizes to the whole of it and slides back towards the lane as far as it will go.

*Measured, nudging off against on, no space requests:* corridor **355,116 → 89,178**, and it is the first phase in which that term has a nonzero multiplicand at all. Crossings **66 → 129** and excess length **648,320 → 1,817,668**, the second following the first since excess is charged per crossing on the edge. Bends 587 → 591, aspect and area untouched. `Cost` improves overall — 1.1629e9 → 1.1549e9 — but only because area dominates the tier (§19). Under real text the corpus goes from 240 shared-segment pairs over 368,902 grid units to **135 over 89,399**.

**Doubling the crossings is partly honest and partly owed.** Two collinear segments do not *cross* under the predicate, so separating them reveals crossings that overlap was hiding. The rest is the ordering key being a projection of one end rather than the full stage: [OWED] the combinatorial stage below, which is what buys path-consistent components minimum crossings with no extra bends.

**The specified stage, still owed.** Nudging retains the combinatorial stage: build the shared-edge graph, assign pseudo-directions by BFS from the lowest-`(channel, edge)`-keyed member (enumerate components by minimum key; on conflict mark a split point and reverse), then insert into each shared edge's order by projecting from the lowest-keyed already-ordered neighbor. Path-consistent components get minimum crossings with no extra bends. Placement assigns **integer offsets `k*gap`** within the channel; segment order is a total integral key `(channel, offset_slot, edge)`, never a partial comparator.
...
**What it does not do is spread the ends out. [OWED, P7c]** All six of `ota`'s bar attachments now land on a long face and three of them land on the same point of it, because nothing distributes ends along a face. That is nudging's, and a fork's fan-out is the same problem as two routes sharing a lane.
...
**And the next round is not worth scoring yet.** A blind reviewer marks the loudest defect on the page, and right now that is a rendering defect rather than a layout one: labels over names (§11.9), routes sharing a lane (§11.5), and — until P7b's own review pass — glyphs sized by text nobody draws (§11.4). Scoring layout through those measures the builder. Tier 0 at zero is the precondition this section named; it is not the only one.
...
*Exit:* better than P4 on the six Tier-2 terms straight-line geometry and no space requests leave nonzero (§11.6) **and no worse than the incumbent** on blind review of the corpus. The second half was misplaced: it belongs to P7 and has moved there (§11.12).
...
On the table's scale the corpus routes 257 transitions over **893 segments**, every one of them axis-aligned, and **68%** of routes turn twice or fewer — 587 turns, which is the table's `bends` row exactly. Under real text: 917 segments, 613 turns, 71%.
...
**P7c — nudging. Landed; channels and congestion no longer motivated.** The phase was three coupled subsystems and is one. **Nudging is built** (§11.5): lanes detected per frame, members ordered by the side they arrive from, integer offsets taken only where the room is known good. **Separator channels are unmotivated** — they were the fix for 496 Tier-0 violations on a synthetic depth-16 shape and the attachment-face rule took those to zero (§11.5). **History-based congestion is unmotivated** — nothing measured demands it, and it is the expensive, sequential, iteration-count-tuned part. Both stay in §11.5 as design; neither is scheduled. What this phase still owes is the combinatorial ordering stage, which is what the doubled crossing count is paying for. The attachment-face rule this phase was going to owe landed early, with P7b's bar transposition that exposed it (§11.5). `w_par` gets its first nonzero multiplicand here: a corridor is a channel the current graph does not have, so `corridor` is identically zero on every scale until this lands (§11.6).

**Two routes sharing a run was what that zero cost.** Nothing separated two edges reaching the same lane, so they drew as one polyline fanning out at its ends — **240 pairs over 368,902 grid units** under real text, invisible to `Cost` because `corridor` was the term for exactly this and was zero on every chart. Nudging fixed and priced it in one move: **135 pairs over 89,399 units**, and `corridor` carries a real number for the first time.
*Exit, met:* the corridor term nonzero and then driven down — 355,116 to 89,178 on the scale these tables use. Tier 0 stays zero on the corpus and on both synthetic 2k shapes, every route keeps its arrowhead, and no leg collapsed.

**P7d — labels and the bench. [OWED]** `PathBox` strip placement by Kakoulis & Tollis matching (§11.9), bounded rip-up on failure, the degenerate-enclosure inflation and its diagnostic, and the bench harness over the two registered routers. Until it lands, a placed box centres on its route's longest horizontal leg and is slid inside the chart rect, and the gap phase 1 widened for it (§11.3) and the leg phase 3 chose need not coincide.

**What that costs is the corpus's largest readability defect, and it is larger than P7c's.** Under real text, **155 of 203 transition labels sit on top of a state box** — worst on `mill` at 70 of 80, `bottler` at 27 of 31, `vac` at 12 of 13. **A second, distinct count: 52 of the 403 inked strings overprint another string**, label on label as often as label on name, which no amount of route nudging separates because nothing moves a label except the route it rides. A shared run (P7c) draws two edges as one polyline that fans at its ends and stays decodable; a label over a state name is not readable at all, which makes this the term most likely to lose §11.12's review.

**The phase tables cannot see any of it**, because `label` is scored from the placed boxes and no space requests means no boxes to place — the same structural zero `corridor` has, for a different reason (§11.6). So the exit below is stated on the rendered scale, and this phase owes the cost golden a second row measured with real text or it is grading itself on a column that is zero either way.
*Exit:* better than P7b on all eight Tier-2 terms, scored under real text — the only scale on which `label` is nonzero (§11.6).
