...
|---|---|
| PB bootstrap, P0 language, P1 model spine, P2 loader, P3 printer and CLI, P4 space tables, P5a metrics + `DrawList` + reference builder, P5b SVG + baseline harness, P5c ABI JSON + Python binding, P6 real layout, P7a router boundary, P7b orthogonal router, P7c nudging, P7d label placement, inflation, and the bench, P7e the combinatorial stage, the attachment, and the element suite, P8 the thread shim, the sharded phases, the seeded structural hash, the determinism class, and `selftest`, P9a the docs commit, `crowd.scav`, the hierarchy-descent scorer, Tier 2 in ems, and profile v5 | **the incumbent comparison**, rescheduled to P9d's exit because a single-candidate render measures the missing search (§11.12) · **routing between two concurrent submachines of one state** (§11.8), which no corpus chart exercises and the element suite pins · rip-up-and-reroute for the boxes no strip fits (§11.9) · **P9b–P9e search, whitespace, and calibration** (§11.10, §11.4), **reordered 2026-09-08** so the level that chooses between whole charts is built first and the fine-grained one last: P9b Level 2 of the portfolio, the phase-1 hoist, and the published latency form · P9c §11.4's compaction and whitespace elimination · P9d the fitted weights and the side-by-side · P9e Level 1 and local search, **gated on a measurement taken there** · P10 viewer · P11 wasm · P12 editor |
...
**[OWED in full: nothing here is built.]** `portfolio_k` and `portfolio_m` are validated and read by nothing (§0, §11.15), and `sweep_count` is read by phase 1's crossing-minimisation sweeps (§11.3) and by nothing else. Everything below is design owned by this section and scheduled as §17's P9b–P9e, **Level 2 first and Level 1 last**: every improvement measured to date is a whole-chart strategy choice rather than a fine-grained ordering move, so P9b builds the level that chooses between whole charts and P9e builds the per-frame level and the moves only if a measurement taken there says a reader can see what they reach; what P9a landed of it is the two-level shape itself, written here before anything was built against it.
...
| search | `portfolio_k`, `portfolio_m`, `sweep_count`, `congestion_iterations`, `ripup_cap`, `spacing_inflation_cap` and `spacing_inflation_increment`. **Three of the seven are read today**: `sweep_count`, which phase 1 spends on crossing-minimisation sweeps (§11.3), and the two spacing-inflation fields, which drive §11.6's retry. `portfolio_k`, `portfolio_m`, `congestion_iterations` and `ripup_cap` are validated and hashed and consumed by nothing until §11.10's portfolio — `portfolio_m` at **P9b** with Level 2, `portfolio_k` at **P9e** with Level 1 — and the stages §11.5 and §11.9 hold as design. **`portfolio_m`**, for §11.10's chart-level portfolio, landed at P9a in `[1, 64]` and ships at 1, riding the `profile_version` bump the em already forced rather than paying for a second output-format change; P9b's move to 4 on both profiles is the change that pays. `sweep_count` bounds every fixed-count improvement loop, phase 1's sweeps included — one knob because they are one question, splittable at **P9d** if calibration wants different numbers |
...
**P9b — Level 2 of the portfolio, and the phase-1 hoist.** Decided 2026-09-08, and the reason is a ratio: every improvement measured so far is a discrete whole-chart strategy choice rather than a fine-grained ordering move, so the level that picks between whole charts is built first and the level that permutes a rank is built last. `portfolio_m` goes 1 → 4 on both profiles, which is a profile change and so an output-format change (§11.6) — `profile_version` 5 → 6. The tuple table is over the chart-global phase-2 knobs: `trybox`, the scale-measure tiebreak, and DAR handed down to each frame from its owner's remaining hole (§11.4), the third landing as a capability and a table row beyond the first four so that M = 8 can be measured before it ships. Each tuple is evaluated whole-chart inside `layout_run` with real routing and exact `Cost` — `argmin(Cost, tuple_index)` in index order, never against a shared best-so-far (§6) — and each candidate runs the spacing-inflation retry as today, the scored geometry being the retried one rather than the attempt that asked for it. **Phase 1 is hoisted out of both loops**: it reads one profile field and no extent, so every inflation attempt today recomputes an identical `SubmachineOrders` (§11.10), and a candidate loop would recompute it M times besides. The fold's second layout is not the same kind of win — §11.4 measured the cut firing on 59 of the corpus's 62 components, so skipping it where it does not fire saves nothing — and what phase 2 can save across candidates is P9e's caching (§11.4).

**What Level 2 buys, measured at P9a: the pick changes on 4 of 11 corpus charts.** Real text, `readable`, the em objective with the shipped weights, over `trybox` × `sm_tiebreak` alone — `axis`, `bottler`, `toolchanger` and `vac`, all four to the LR packer alone. `axis` bends 25 → 16, corridor 3,796 → 0, `label` 2 → 0, area **−13%**; `bottler` corridor 97 → 0, `label` 18 → 3, area +39%; `toolchanger` corridor 8,653 → 1,562, `label` 4 → 1, area +35%; `vac` `label` 2 → 0, area **−13%**. Adding Level 1's `sweep_count` row to the same grid adds only `mill` and `ota`, and nothing yet says P9e's bounded moves buy anything a reader sees. That is the whole argument for this order.

**M scales down by a closed form from `entity_count`** — never a wall clock and never a thread count (§11.10) — `m = max(1, portfolio_m >> max(0, ilog2(entity_count) − 9))`, unless the implementation argues a better constant. `entity_count` is states plus submachines plus transitions, the same sum §6's sharding reads, so M reaches one candidate at 2,048 entities: every corpus chart runs 4, the biggest of them an order of magnitude short of the first shift, and both 2k charts run 1, the flat one carrying 4,223. **That form is the published latency claim**, so 2k latency is a formula rather than a number somebody repeats per machine.

**Goldens: the structural and coordinate hashes move only on the charts whose pick changes, and `inputs` moves everywhere** because the profile did. `corpus_routers.txt` and `cost_terms.txt` score the phases directly rather than through `layout_run`'s pick, so **neither may move at all** — which is what says the routers and the scorer are untouched by the search wrapped around them.
*Exit:* the four charts above move as measured; the determinism class and `selftest` green at {1,2,3,5,8,13,16} with M = 4; **the pick per chart asserted as an index, not a hash**, a hash saying that something moved and localising nothing, plus a fixture where two tuples tie on `Cost` so the index is what decides; and **full-layout latency at 2k published as the closed form above** (§11.11's bet), the first of P9's original three clauses.

**P9c — compaction and whitespace elimination.** §11.4's two owed LR-rectpacking steps (Domrös et al., IVAPP 2021), taken here rather than after calibration because **the visible price of every legible candidate is whitespace** — under real text `brew` +40%, `crowd` +56%, `toolchanger` +35% and `bottler` +39% area — and these two steps are the published, bounded, search-independent answer to exactly that. Every em² they reclaim makes the legible candidate cheaper under any weights, so they land before the weights are fitted rather than after. **Compaction is its own commit**: order-preserving, `sub_sep` preserved between two placed siblings, Tier 0 zero by construction. **Whitespace elimination is its own**, because enlarging a box to fill its row is an appearance decision and the side-by-side judges it separately from reclaiming a gap nothing occupies.
*Exit:* `vac`'s root frame occupancy above **54.7%**, with the number stated rather than "improved" (§11.4); area down on every corpus chart or the exception named; every element-suite property still holding; rendered and looked at (§11.12).

**P9d — the weights fitted, and the side-by-side that has waited since P6.** The weights are fitted to `tools/audit.py`'s reader-visible counts — a label over a state or another route, a label nearer a stranger's line than its own, an overprinted string, an arrowhead over another route's end — over the candidate set P9b's Level 2 generates and **on P9c's canvases**, because fitting before compaction fits against canvases about to shrink. Then the unblinded page decides (§11.12): the counts choose which candidates are worth looking at, and looking is what settles it. **`crowd.scav` is the case that says the weights are wrong and that no search fixes it.** A label overlap costs 24 and an em² of canvas costs 1, so under the em the cramped drawing still wins — `t2` **1,230** shipped against **1,438** for the candidate whose every audit count is zero — and `brew` loses the same way. That is the weight fit, so it is this sub-phase: the pinned `label` **4 must go to 0**, and the carve-out in `functional_drawlist_tests.cpp` moves with it (§5). `sweep_count` splits here if calibration wants a different number for phase 1's sweeps than for search's (§11.15), and §11.3's global sifting and edge-weight schedule becomes judgeable for the first time.
