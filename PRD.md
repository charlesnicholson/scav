...
Reconciled against the tree on 2026-09-09. Phase names are §17's.

| Built and green | Owed, with an owner section |
|---|---|
| PB bootstrap, P0 language, P1 model spine, P2 loader, P3 printer and CLI, P4 space tables, P5a metrics + `DrawList` + reference builder, P5b SVG + baseline harness, P5c ABI JSON + Python binding, P6 real layout, P7a router boundary, P7b orthogonal router, P7c nudging, P7d label placement, inflation, and the bench, P7e the combinatorial stage, the attachment, and the element suite, P8 the thread shim, the sharded phases, the seeded structural hash, the determinism class, and `selftest`, P9a the docs commit, `crowd.scav`, the hierarchy-descent scorer, Tier 2 in ems, and profile v5, P9b Level 2 of the portfolio, the phase-1 hoist, DAR as a capability, and profile v6, P9c the spot list, whitespace elimination always, and compaction as Level 2's bit 1 | **the incumbent comparison**, rescheduled to P9d's exit because a render measures the search behind it, and at P9b that is one of two levels with the weights still unfitted (§11.12) · **routing between two concurrent submachines of one state** (§11.8), which no corpus chart exercises and the element suite pins · rip-up-and-reroute for the boxes no strip fits (§11.9) · **a label sliced by a region divider** (§11.10f), which `Cost` does not price and the composed search found two of · **CI's run time under the shipped search** (§11.10f), with `selftest` at seven corpus passes a call · **§11.10g's plan**: alignment across fold pieces, layering-axis compaction and rank-gap sizing, port order, label rules, frame orientation with top and bottom ports (which regions stacked one above the other need for their divider ports), fold direction, and a fixture that seals for the inflation retry · **P9d's six label and attachment defects** (§11.9.3), found on the page 2026-09-09 and **ahead of both P9d and P9e**: §11.9.4's anchored placement, I3 as a feasibility constraint, rip-up for the labels their own line slices, the mis-tiered lane rules, and the boundary-node corners — the endpoint exemption and the fold's discarded label charge are landed · **P9d–P9e calibration and the fine-grained level** (§11.10, §11.4), the order **set 2026-09-08** so the level that chooses between whole charts was built first and the fine-grained one is built last: P9d the fitted weights and the side-by-side · P9e Level 1 and local search, **gated on a measurement taken there**· P10 viewer · P11 wasm · P12 editor |
...
**Two search fields are validated and read by nothing.** `congestion_iterations` and `ripup_cap` are bounds-checked at load and hashed into the inputs digest (§6), so changing one moves a golden's inputs column without moving a coordinate. `portfolio_k` was the third, and is read by Level 1 since 2026-09-19 — shipping at 1,024 since 2026-09-20, which is two thirds of the way to where every corpus chart's greedy search converges (§11.10). `portfolio_m` was the fourth: it landed at P9a on the profile bump the em already forced and went live at P9b with Level 2, shipping at 4 and re-bounded to `[1, 8]`, and **P9c takes the shipped value to 2**, rows 2 and 3 being compaction's and unjudgeable until the weights are fitted (§11.15). The other two belong to stages §11.5 and §11.9 hold as design and nothing has scheduled. `sweep_count`, `portfolio_m` and the two spacing-inflation fields are what the tree reads of that group (§11.15).
...
**`libscavlayout` uses a documented standard-library subset** — `<cstdint>`, `<bit>`, `<limits>`, `<vector>`, `<array>`, `<utility>`, `<type_traits>`, `<cstring>`, and `<algorithm>` for its simple predicate queries (`find_if`, `any_of`, `count_if`) — and nothing else. Every sort, hash, and container with iteration order that reaches output is scav's own (above): the algorithms that decide an order, `std::sort` and `std::partition` and their kin, are banned by name. The subset is enforced by an include-check in CI, so "bring your own compiler" does not quietly mean "bring your own conforming sort".
...
**Measured 2026-09-13, and that cache is not worth building.** Timed inside `size_pass` over the corpus: `cross_coordinates` is **95–96%** of phase 2, packing is 3–4%, and everything the paragraph above proposes caching — extents, `layer_w`, `layer_h`, area, target, chunk boundaries — is **1–2%** together. The invariants are real and caching them buys nothing.

**Re-sizing one frame's spine is worth about 2x, not the order of magnitude it looks like.** `size_pass` already walks bottom-up by level, so a permutation of one frame needs that frame, the state owning it, the frame that state sits in, and so on to a root — no sibling subtree. Measured as a share of one chart's whole `cross_coordinates` time: `mill` 17% mean, `bottler` 47%, `vac` 40%, `tcp` 65%, `ota` 82%. **Every spine ends at the root frame and the root is usually the largest**, so the floor on an incremental re-size is whatever the root costs, and on a shallow chart that is most of it.

**What is left, and it is the thing Level 1 should be built against.** A spine stops early wherever a frame's box comes out unchanged, and an in-rank permutation cannot move a chunk's *width*: `layer_x` accumulates `layer_w + rank_sep + boundary_gap` and all three are order-invariant, so only `chunk_h` — the tallest `centre + extent/2` Brandes–Köpf produces — can move, and `layer_h` bounds it whichever order it runs in. On an unfolded frame the box's width is therefore invariant by construction. On a folded one the packing can still move both, and the cut fires on 59 of 62 corpus components, so that narrowing buys less than it sounds. **The measurement to take before building anything is how often a real in-rank move changes a frame's box at all**, which needs Level 1's move to exist and is where P9e should start.
...
**The face rule is local, and a better local rule is not the fix. Measured 2026-09-20 and reverted.** `ortho_escape_horizontal` picks a face by which separation to `toward` is larger, and it is handed a point and one box, so it cannot know how many bends the route will need. It is implicated in three reader-visible defects: `gauntlet/fork`'s branches leaving through a 64-unit bar cap, and both of `estop`'s odd routes.

The obvious fix was tried: decide the two faces **as a pair**, from the two boxes. Two boxes whose seatable spans meet across an axis can be joined by one straight line, two that meet across neither need an L, and picking the same axis at both ends of an L is a Z that costs a second bend for nothing. On `estop` it does exactly what it says — `Tripped → Latched` goes **2 kinks to 1**, and the chart's real-text Tier 2 falls 3,073 to 2,509.

**Over the corpus it is a clear loss**: Tier 2 **+8.8%** on the no-space goldens and **+4.1%** on real text, with bends up 280 → 313 and 268 → 291, and nine charts of eleven worse. Narrowing it to only the L case changes nothing — **the L case is the whole regression**, which is the finding. A straight corridor is only straight if nothing stands in it, and an L only saves a bend if the face it demands is not already wanted by another net. Neither is knowable from two rects.

**So the face is a search dimension, not a rule.** It is a small discrete choice per net — four faces per end, and the pair decision above is a good *proposal* for §11.10's guided generation — scored on the exact objective like any other move, where blockage and contention are priced rather than guessed at. That is the fix; another local heuristic is not. `[OWED]`
...
w_crowding   * crowding / em                                  -- lanes closer than one em, below
```

**Crowding is `corridor`'s near neighbour, and it was the term the search found missing.** Two segments of different transitions on one axis, overlapping along it, closer together than one em (`font_size_grid`) but not on one line, are charged `along × (em − apart) / em`: the overlapped length, scaled by how far short of an em apart they fall. At `apart = 0` that is the whole shared length, which is what `corridor` charges, so the two meet at their boundary instead of leaving a gap between "collinear" and "fine"; at `apart = em` it is zero. It prices exactly what the audit's `lanes closer than one line of text` counts (§11.12), and the em is the same pitch nudging spreads lanes to (§11.9.5).

**Why it was needed, measured.** Adding the face a transition leaves by as a search dimension (§11.10e) took Tier 2 **−10.2%** on the no-space goldens and **−6.0%** on real text with bends down 280 → 254, and took the reader-visible audit **46 → 55** at the same time — lanes closer than a line of text **26 → 32**. The search had a new lever and nothing priced two lines a few units apart, so it spent the lever there. A stronger search on an objective with a hole in it finds the hole; this closes that one.

**Its own weight rather than `w_corridor`'s**, although the two are continuous: the fit (§11.6, `tools/fit.py`) should be free to price a lane that is merely tight differently from one that is shared, and a term folded into another can never be.

**The weight is an exchange rate, and a sweep found it.** Shipped first at `w_corridor`'s 48, crowding moved nothing — lanes closer than a line of text read 32 with it and 32 without. That is the difference between a term the search cannot act on and one it will not, and a sweep tells them apart:

| `w_crowding` | tight lanes | kinks | route segments |
|---|---|---|---|
| 48 | 34 | 289 | 546 |
| 192 | 18 | 291 | 548 |
| **512** | **9** | **296** | 553 |
...
(Counted from the geometry, one pair per tight pair, so it runs a little above the audit's figure.) The response is monotone, so it was the exchange rate. The knee is between 192 and 512: the first step removes eight tight lanes per kink it costs, the second about two, the third under half of one. **It ships at 512, which is `w_bends`**, and reads plainly: an em of lane run at zero separation costs what one bend does.

**What it bought, on the reader's scale.** The corpus audit reads **27** defects, against **46** before the face a transition leaves by was searched and **55** with that dimension on and crowding unpriced. Lanes closer than a line of text **26 → 6**, routes sharing a run **1 → 0**; corpus kinks **311 → 296** and route segments 568 → 553, against the same budget of 1,024 before either change. `gauntlet/crowd`'s `label_near` falls 346 → 77, and `gauntlet/fork`'s branches leaving through the bar's 64-unit cap go 2 → 1. One class rose: a foreign line inside a label's leader, 2 → 4. **Tier 2 is not compared across this change**, because the objective gained a term.
...
**The weights are fitted. P9d, landed 2026-09-13.** They were hand-set at P9a, identical at both profiles, and never measured. The instrument is `tools/fit.py` against `tools/candidates.py`'s 21 charts × 8 rows: every candidate's term vector beside the defects a reader counts in the render of it (§11.12). **What is fitted is the deployed metric** — the defects of the row `argmin(Cost, row)` takes — and not a regression on counts, and not pairwise ranking agreement either: a vector can order most pairs right and still put the wrong row first, which is the only comparison `layout_run` makes. Pairwise scored 200 of 241 and picked 98; fitting the pick directly scored 161 pairs and picked **93**, against a floor of 88 and 111 for the shipped weights.

**Three weights moved and six did not, which is the finding.** `w_bends` **64 → 512** is 16 of the 23; `w_label` **24 → 768** and `w_excess_len` **4 → 0** are one each. Every other term's shipped value costs nothing or is free: `w_corridor`, `w_crossings`, `w_label_near` and `w_aspect` sit inside the range that holds the fit, `w_adjacency` is unconstrained by the corpus at any value in `[0, 1024]`, and **`w_area` is pinned at exactly 1** — the one weight the data will not move. A human set six of nine correctly.

**`w_excess_len` was pulling against area, and that is a sign error rather than a magnitude.** Excess length is route length past the straight-line minimum, so a *compact* drawing forces detours and a sprawling one lets routes run straight: penalising it rewards sprawl. Keeping it at 4 costs one defect **and 5.5% of the canvas**; zeroing it is better on both. The defensive instinct to keep a term "for safety" was wrong here and the measurement is why.

**And more search was making the picture worse.** At the shipped weights, running all eight rows instead of two reads **88 defects against 78** — the objective was wrong enough that giving it more candidates made it choose worse, and `portfolio_m = 2` was protecting the corpus by accident. Fitted, eight rows read **71 against a floor of 70**, so Level 2 is now cashed: `portfolio_m` ships at **8**. Corpus 78 → 71, the element suite 19 → 18, canvas −0.3%. **A quarter of the in-sample gain generalises**: leave-one-chart-out reads 105 where in-sample reads 93 and shipped reads 111, which is what eleven effective charts and nine parameters buy and is the reason to widen the corpus before fitting again.
...
#### 11.9.3 Labels are anchored and contained, and six defects say they are not

**Reviewed on the page, 2026-09-09, against the shipped pick of every corpus chart** (§11.12's standing instrument, driven by `tools/candidates.py`). Six absolute defects came out of it. **Five were invisible to both the audit and `Cost`**, so the numbers this phase had been reasoning about were precise about the wrong things: reader-visible defects read 100 shipped while the instrument was blind and **227** once it could see, of which **200 are labels**. Not one of the six is anything §11.10's search could reach — a defect present in every row of the table is the pipeline's, and `estop`, `led` and `ota` are byte-identical on all eight.

**Four invariants, and they are Tier 0 in shape rather than Tier 2 terms** (§11.6). A cost term is an exchange rate, and `area` at 45% of the weighted sum buys any of these for the price of a slightly smaller canvas — which is exactly what it does today:

- **I1** No rendered item occupies the space between a transition and its label.
- **I2** A label hangs off its own polyline. Absolute, not "nearer to it than to somebody else's": the relative test fires only where another route is nearer, so an orphan in empty canvas scores zero. `axis`'s `twist release` sits **seven text heights** from its nearest own leg, and **68 of 203** corpus labels are past one.
- **I3** Everything of a submachine is contained in its parent state's box, out-of-machine transitions excepted. The only containment class was the chart rect; **24 of 203** labels are outside the state enclosing their own transition.
- **I4** A label overlaps no state box, **its own endpoints' included**. `26 of 203`, of which 23 were exempt.

**The exemption was in three places and the reason belonged to none of them.** "A state enclosing an endpoint is the composite the label lives inside" is right, and its justification is reachability: a transition running inside composite `C` must have its label inside `C`, so charging it there makes zero unattainable. An endpoint encloses nothing, so the reason does not reach it — and all three sites marked the endpoint anyway. `cost.cpp` and `tools/audit.py` are fixed (`label` 3 → 26; `crowd`'s pin 4 → 6, the count having been short rather than the drawing having moved). **The third was this section's own feasibility rule**, which excused a candidate overlapping "a state that encloses neither endpoint" — so the placer was permitted to put a label on the box its transition names, and did. **Landed**, with the fold charge below, because either alone reads worse than both: a refusal makes strips infeasible and raises the centred fallbacks on its own.

**A label charged to a rank boundary loses its room when the fold repacks that boundary.** `order.cpp` charges each label's width to one rank boundary — the middle of the route (§11.3) — and a probe confirms it works: widening one label reflows the chart. But §11.4's fold packs the ranks it folds as *rectangles*, `pack_best(pieces, node_sep, …)`, and a boundary the fold turns into a gap between two packed pieces is separated by `node_sep` and nothing else. `estop`'s `held` is 384 wide across a 288-unit gap and overlaps `Tripped` by exactly 96; `led`'s `charged` is 672 across the same 288 and overlaps two states. **This is why I4 was violated on the two simplest charts in the corpus.** **Landed**: the shortfall a cut discards rides the new chunk's leading edge, so it is room wherever the packing puts the two pieces. Both charts are clear, and over the corpus `label over a state box` reads **26 → 11** and is now equal on all eight rows — what is left is structural rather than a placement anybody chose. The centred fallbacks went **16 → 7 of 192**, the room the fold gives back more than paying for the refusals above.

**What the pair cost, and it names the next item.** Containment went **24 → 39** and detachment **68 → 76**: a box refused a state's rect takes a strip further from its own leg, and sometimes outside the state its transition runs in, because the feasibility set holds I4 while I2 and I3 are terms. **I3 has to join it, and I2 is answered by §11.9.4's anchor rather than by a bound.** There was a choice in I2's shape — strips are one box height apart, so a hard "within one text height" admits only the nearest and pushes boxes into the fallback, which is a guaranteed slice; a lexicographic key ahead of the `label_near` shortfall reduces detachment without manufacturing one. `[OWED]`

**The fallback was never built, and building it took the slice class to zero. Landed 2026-09-19.** §11.9.4 says a box with no clear candidate **keeps its anchor and accepts a collision**; what `place_labels` kept was `centred()`, the leg's own midpoint, which is not anchored and rides the line. So every fallback was a sliced label *by construction* and the two counts were one number because they were one thing. A second tier now holds the best candidate that is anchored, inside its enclosing state and uncut by its own route, but may overlap a state, a text band, a settled box or a foreign route — the strict tier's candidate where there is one, that where there is not. **Centred fallbacks 6 → 0: every one of the corpus's 192 placed boxes is anchored.** Own-route slices 6 → 0, divider slices 0, `label over another route` 4 → 0, `a foreign line inside a label's leader` 8 → 4; `label over a state box` 2 → 6 and overprinted text 1 → 3, which is the collision the section says to accept. Corpus **68 → 60**, element suite 18 → 16.

**One thing to get right, and it cost 26 defects to get wrong.** The candidate sweep prunes against the best key held so far. With two tiers the bound must be the *strict* one: the loose tier holds the best of a superset, so pruning by it discards candidates the strict tier still wants and pushes boxes into the fallback that never needed it. Pruned that way the corpus reads **86** against 60.

**Rip-up-and-reroute is no longer what this class needs.** It was owed because a box with no strip had nowhere to go; it has somewhere now, and the somewhere is a placement rather than a reroute. What remains owed is only the case a collision is genuinely worse than a detour, and nothing in the corpus is now that case.

**A line drawn through a label cuts the word in half, and whose line it is makes no difference to the reader.** Reviewed on `led`: `charged` is cut between "cha" and "rged" by the very transition it names. **This is the centred fallback and nothing else** — this section's strips sit *beside* a leg, so a box that reaches one is never crossed by it, and a box with no feasible strip keeps the leg's exact centre with the leg through it. The two counts are therefore the same count: **12 of 192 centred fallbacks, 12 of 203 sliced labels**, having read 7 and 7 before the exemption narrowed to states enclosing both ends. So the fallback is not a neutral last resort and must not be one — a box with no feasible strip needs somewhere else to go, which is what rip-up-and-reroute of that one edge is for. A dotted concurrency boundary slices the same way and is neither a route nor a state, so no test above sees one; it reads **0 of 203** at the shipped pick and on the element suite — but **9 of 203 at row 0**, so it is a violation the portfolio already produces and `trybox` happens to dodge, not one nothing reaches. Own-route slices read 7 at the pick against **14 at row 0** the same way. `[OWED]`

**The trunk exemption is a common destination, not a common anything. Landed.** `trunk_of` excused a common *head* as well as a common tail — a fan-out leaving one state — and excused however much suffix happened to coincide rather than the final leg. Two routes leaving one state as one line read as one line going somewhere ambiguous, which is the whole defect; only a shared destination gives a reader something to follow them to. The head is no longer counted and the tail caps at one shared leg. **What it costs is a much larger `corridor` and no better drawing yet**: `bottler` 3,616 → 29,584, `mill` 1,152 → 20,290, `axis` 0 → 4,272, the nested 2k 139,984 → 396,000 and the flat 11,680 → 992,208. Every audit count is unchanged, because no row of the table avoids the sharing — this prices a violation the portfolio cannot dodge, so it is a measurement change that will matter at the weight fit rather than a fix. **Still owed**: the exemption should be *low-weighted* rather than free, and free is what it is; a distinct weight for the shared arrival is a profile field and so an ABI break, which waits for the fit. `[OWED]`

**Two lanes closer than the type they carry, and the class that could not see them.** `ortho_clearance` is `node_sep / 3`, 96 units at `readable` against a 160-unit text height, so a legal pair of lanes is narrower than the glyphs beside them — which is what `mill` reads as unfollowable. **`routes share a run` never counted these**: it counts segments that are *collinear*, and these merely run alongside. A class for it reads **89 pairs over the corpus**, the largest single count this review has produced.

*Measured 2026-09-10, and the obvious lever is the wrong one.* Raising the clearance to a whole em addresses **12 of the 89** and costs eight containment violations, three more slices and three more labels over a route — 192 reader-visible defects to 190, which is noise for a change that large. **Area is not the objection** (§11.11: correctness first, and a bigger picture is the price of it); the objection is that the clearance is not where the crowding comes from. It insets attachments and rings as much as it separates lanes, and 77 pairs survive it. What the finding wants is lane separation in §11.5's nudging, which assigns the lanes in the first place.

*Measured again 2026-09-11, and located.* The pairs sit **39 to 154 units apart, median 121**, against a 160-unit line — near sufficient rather than wildly short. `ortho_spread_attachments` pitches a face's seats by `imin(clear, len / 3)`, under a line at every shipped profile. Raising that floor to a line of text was tried and **reverted**: it reaches seven of the 89, and it costs `regions.scav` a 1,120-unit shared run at `readable` where the element suite pins zero.

**Because the pass is not where they are.** Of the pairs, **one** is two seats on one box, which is all `ortho_spread_attachments` ever sees; **17** are terminal legs on *different* boxes running alongside; and **64 involve a leg in the middle of a route**. The owner is §11.5's nudging, which assigns lanes in a corridor — which is what this section said before the attachment pass was tried, and the attempt is recorded because the shape of the count is the argument: crowding is a corridor property, not a face property. `[OWED]`

#### 11.9.5 Phase 2 reserves what phase 3 and §11.9 need

**Two lanes carrying type may not sit closer than one line of that type**, and a label needs room beside the leg it is anchored to. Neither is reservable in phase 3, which has only the space phase 2 left it, and phase 2 reserves neither today — it charges a label's *width* to one rank boundary (§11.3) and nothing else. That is why this phase keeps meeting one wall: §11.9.3's placer refusals, §11.9.4's eighteen fallbacks and the 89 crowded pairs are one scarcity seen three ways.

**The two reservations.** Per rank boundary, `lanes × line height`, `lanes` being the edges crossing it — so the pitch §11.5 wants is a pitch the corridor can hold. And beside each leg, `leader + box height`, which is §11.9.4's anchored label and the perpendicular half §11.3's charge has always lacked.

**They land together, and the pitch lands with them.** A pitch floor with no room to spread into produces fallbacks; room nothing spreads into is canvas. This phase has taken that trade three times — the placer without the fold, I4 without I2 and I3, the clearance without the pitch — and each time the violation moved rather than went.

**Built 2026-09-11, and it took the corpus 168 → 110.** The corridor charge sits beside §11.3's label-width charge in `order.cpp` and takes the max of the two, because both say how wide one corridor has to be. The leg charge is cross-axis and rides `size.cpp`'s `extent`, the slot `cross_coordinates` separates by rather than the rect a reader sees, so half lands each side of each end and two adjacent slots open a whole `leader + box height` between them. Per class: crowded lanes 56 → 44, `nearer another route` 52 → 29, own-route slices 19 → **5**, sliced by a divider 1 → 0, over another route 11 → 2, shared runs 9 → 4, overprinted text 2 → 0. Containment went the other way, 2 → 8, which is I3 still being a term rather than a test — with room to take, a box takes a position outside its state.

**Three corrections the measurement forced, each recorded because the naive form looked right.** *Per component, not per frame*: phase 2 lays a frame's components out separately and packs them, so edges in two components never share a corridor; summing them reserved a width neither needed and un-sealed `sealed_chart`, whose whole shape is a zero-width boundary. *The two boundaries an edge turns in, not every boundary it crosses*: a route changes cross position where it leaves its source and where it meets its destination, and runs straight between — charging every boundary read fourteen lanes at each of a 256-chain's boundaries where three turn there, 960k grid units of corridor against a 524k domain. *The whole `leader + box height` and not its shortfall against `node_sep`*: the shortfall looks like the same room for less and reads 133 violations against 110, because a leg leaves its node's centre and what the shortfall opens is the gap between two node edges.

**What it cost, and the guardrail moved.** Corpus area 9.67M → 18.36M pt², against PlantUML's 13.78M and elkjs's 6.54M — so scav is now the largest of the three where §11.12 had it the smallest. The 2.3× headroom that section recorded is spent. This is the trade the goal set on 2026-09-11 asks for and it is taken deliberately, but it is a debt against §11.12's exit criterion and **search is what pays it**: every row of §11.10's table now lays out with the reservation, so an arrangement that needs less of it is a point the portfolio can already reach. *The knee is measured*: the corridor charge alone is 15 violations for +0.5% area, the leg charge is the other 43 for +89%, and doubling the leg charge buys 4 more for a further 14%.

**Eleven of the fourteen box overlaps were the instrument, and the placer and the drawing never disagreed.** Checked because I4 is a feasibility test and fourteen labels were still overlapping a state, which would have meant `place_labels` and the builder holding different geometry — and would have quietly corrupted the I1 and I3 tests owed next. They do not. Eleven of the fourteen are self-transitions, whose label nothing places: the builder draws it in the `after` band its own source reserved for exactly it, which is inside that source's rect. §11.12's audit meant to excuse exactly that — the 32-of-203 reading recorded at P7c already did — but the exemption sat inside the branch for states enclosing *both* endpoints, and `enclosing` is strict, so a source is never among its own ancestors and the clause could never fire. The band above it, the state's own title, is still out of bounds and still checked. The other three are the centred fallbacks, which §11.9.4 has accepting a collision on purpose, and they are the same three the slice class already counts. **So the class is 3 of 203, every one of them rip-up's, and the honest corpus total is 99.**

**I3 is a test, and I1 turns out not to be a placement question at all.** Built 2026-09-11 on the reservation above, which is what made refusing affordable. *I3*: a label whose transition runs inside a composite is bounded by that composite and not by the chart — the states enclosing both ends nest, so intersecting them all with the chart rect is the innermost of them without having to order them by depth. `label outside its enclosing state` **8 of 203 → 0**, for three more centred fallbacks, 5 → 8. *I1*: measured before it was built, and the measurement said not to build it. On every one of the corpus's 192 placed boxes, **zero** have a foreign leg strictly nearer than their own — attribution is already exact, and `better()` has ordered the `label_near` shortfall ahead of everything else since §11.9.3, so the placer was already maximising foreign clearance. Held as a hard test the class does not move: 28 either way, because the 28 are candidates where no placement has more room. What it *does* do held hard alone is take the fallbacks 5 → **43**, which is the third time this phase has priced a prohibition with nowhere to go. **So the residual 28 is a margin rather than an attribution**: the audit reads a violation where a stranger's line is nearer than a label's own plus one em, and under §11.9.4's anchor a label's own gap is the leader exactly, so what the rule now asks is that no foreign line come within `leader + one em` of a label. That is a crowding rule wearing an attribution rule's name, it is the same quantity §11.9.5 reserves beside a leg, and it is reachable by routing rather than by placing. `[OWED]`: restate the class against the anchored model, or leave it to rip-up.

**Re-measured 2026-09-19 at 37, and it is not scarcity.** Of the 37 crowded pairs the corpus now draws, nudging's own accounting says:

| | pairs |
|---|---|
| no lane holds the pair at all | **27** |
| in a lane with no room to spread | 7 |
| in a lane with a pitch or more of room, and not taken | 3 |

So the residue is still a *grouping* question rather than a spacing one, and the earlier reading — "53 of 56 have room" — has been overtaken: the pairs with room have mostly been taken, and what is left is pairs nudging never forms a lane for. **31 of the 37 involve a leg that is terminal in the composed polyline** (24 with both ends terminal, 7 with one), and a lane member must be an *interior* segment because nudging moves a segment by dragging the legs either side of it. A terminal leg has a seat instead of a leg, so there is nothing to drag.

**Which puts the class back where §11.10a's seating pass left it.** `ortho_separate_attachments` moves seats and reached 3 of them; sliding a seat inside nudging reached none. The 7 with no room are `dock`-shaped: a corridor one line wide, flush along a box border. The 3 with room and not taken are `known_good` refusing a move that would newly cross something. **None of the three groups is answered by more nudging reach**, which is what this section has been assuming since it was written. `[OWED]`

**The class the reservation did not reach is 44, and 43 of them are nobody's.** Located 2026-09-12, by the leg each pair sits on. **22** are two *terminal* legs on *different* boxes running alongside; **12** have one terminal leg; **9** are both mid-route; and **1** is two seats on one box, which is the only one `ortho_spread_attachments` can see. Nudging takes a segment perpendicular and drags the legs either side, so a leg whose end is seated on a box is excluded by construction — moving it would take the seat off the border. **So 34 of 44 need a mechanism that does not exist. Built 2026-09-19 and it is a seating pass, not a nudging one.** `ortho_separate_attachments` runs last of the four, and is the only one that compares a seat on one box with a seat on another: it keys each seated leg by the coordinate it *runs at* — a left or right face leaves along x, so the leg sits at the seat's y — sorts by that, and pushes apart any adjacent pair closer than one line of text whose runs overlap. Each seat moves along its own face, half the shortfall each, clamped by `onto_face`.

**Nudging runs once more over the composed polylines, because a lane is what a reader sees and a reader sees neither frames nor segments.** Found on `dock`: `Charging -> Solid` runs down a line `Solid -> Off` runs up, 1,777 units of it in opposite directions, and a merged run is only legal into a *common destination* — here one route's source is the other's target. The per-frame pass could not reach it twice over: a decomposed transition's pieces are routed in different frames, and the piece carrying the down leg is three points long, so that leg is terminal within its own net and no interior segment holds it. Over the composed polylines it is interior and the two are one lane.

**Each transition is bounded by the innermost state enclosing both its ends** — the frame it was routed in, and the chart where there is none. A crossing transition is bounded by the ancestor it crosses inside, which is what lets its pieces leave the child frames; one wholly inside a composite may not leave it. Bounding the pass by the chart instead pushes a route out of its own composite, which `route_tests`' eight-unit box reads at once. `nudge_lanes` therefore takes a bound per net rather than one for the frame, and a lane's room is the intersection over its members'.

**Corpus 60 → 59, element suite 16 → 15, and `gauntlet/cycle`'s shared run 650 → 0.** `NudgeStats::lanes` now counts a lane once per pass that examines it, so the per-frame and chart-wide passes each report the one they saw. **And `dock`'s own pair does not move**: the lane is found, and its room reads −1 up and 0 down — it runs flush along one box's border with another a unit away. That is a corridor one line wide, so what it needs is somewhere else to route rather than a nudge. `[OWED]`

**Three exemptions, and two of them were measured rather than reasoned.** One net's own two ends are what alignment just put on one coordinate. Seats sharing a box *and* face are the spread's, where a trunk lives. And **two nets that meet at a box at all are one fan**: pushing apart the ends they do not share bends the fan and crowds the end they do, which `gauntlet/fanin` reads as **five crowded pairs against one**. Nets phase 1 gave a corridor are *not* exempt, though the leg model is only approximate for them — excluding them reads 73 on the corpus against 68, so the measurement keeps them in.

**Corpus 71 → 68, element suite 18, canvas unchanged.** Crowded pairs 41 → 38, and the canvas does not move because what this pass moves is seats rather than boxes.

**And it does not reach as far as the count suggested.** 34 pairs involve a seated leg and the pass takes 3 of them. The rest are pairs it exempts — fans, and seats already at their faces' limits — so the honest reading of the earlier location work is that "involves a seated leg" was necessary and nowhere near sufficient. `[OWED]`: what is left of the class after this.

**A seated leg nudges by sliding its seat, and that is the design.** The exclusion is sound only if a seat is a *point*, and it is not: a seat on a box's left face may sit anywhere along that face's straight zone, so the displacement that moves the lane moves the seat along the border with it and the drawing stays attached. A lane member is a segment with a leg either side; for a seated end the **face replaces the leg**. Three things change and nothing else does:

- **Travel.** A free end is bounded by its leg turning round. A seated end is bounded by the straight zone — the face's extent less the drawn corner arc at each end, `state_corner_radius` (§11.9.5), because a seat under the arc points at blank canvas. The face must be perpendicular to the displacement, which it always is for a member: a route leaves a vertical face along x, so the segment is horizontal and the slide is in y.
- **Crossing votes.** A free end votes for the order that keeps its leg from crossing a neighbour's segment. A seated end has no leg on that side and crosses nothing, so it casts no vote.
- **The checks.** `known_good` reads a four-point window and tests three segments; a seated end drops its neighbour, so the window is three points and two segments. The collapse guard and the leg-extent guard apply to the legs that exist.

**What it does not do, on purpose.** It does not re-seat onto another *face* — that is `ortho_reface_attachments`, run before this and on different evidence — and it does not move a seat past another seat on the same face, which the obstacle checks already refuse by way of the bumper. `nudge_lanes` gains the obstacle each net's head and tail is seated on (`INVALID` for an inner-face end, §11.14) and the `corner` array phase 3 already carries parallel to `obstacles`.

**Built 2026-09-12, measured, and not kept. This is the finding.** The mechanism works: 453 of 1,001 lane members across the corpus are seated ends, and 56 of them slide. **It separates none of the 23 pairs it was built for** — both-terminal reads 23 before it, 23 with it, and 23 with it at every spread pitch tried. What it does instead is add members to lanes that were already tight, which takes the crowded class 44 → 47, one more centred fallback and one more label inside a leader. So the premise — that the 34 are untouched because nudging will not move a seated leg — is **wrong**, or at least not the whole reason: a seat that can slide still does not end up a line of text from its neighbour. The candidates left are that the two boxes are themselves closer than the pitch, which is phase 2 and not phase 3, and that the pair wants two different *faces* rather than two positions on one. **Reverted, and the design stays written here** so the next attempt starts from the measurement rather than from the idea. `[OWED]`, now with a reason to doubt it.

**And the step is a fraction of the pitch on purpose, which was the second thing measured and not kept.** The spread is `min(gap, window / (groups - 1))`, so a lane with a short window moves every member by *less than a line of text* — which separates nothing a reader can tell apart, and it is tempting to read that as waste. It is not. Held at the full pitch the corpus improves, 44 crowded pairs → 42, and `gauntlet/cycle.scav` **goes from zero shared run to 473 units at `readable` and 970 at `compact`**: where the window is short nothing moves at all, and two routes stay exactly collinear. A fraction of a pitch is worse than a pitch and better than nothing, because the classes are not equally bad — two lines a reader cannot *separate* are worse than two a reader can only just separate. Refusing instead of spreading reads 44 and 8 shared runs against 44 and 4. **The lever on a short window is the window**, which is §11.9.5's reservation, and not the step.

**A lane is a union, not a run. Landed.** Members sort by `at`, so `lo` is not monotonic, and the run testing `lo < reach` as it advanced was wrong both ways: it stopped at the first member whose extent did not reach, leaving a third that overlapped the first in a lane of its own, and `lo < reach` is one-sided against a union, so it joined members whose extents never met. Unioned over pairs within `gap` instead. **The corpus does not move at all** — a run reads differently only where a lane is crowded enough for a third member to sit behind one that does not overlap — and the 2k targets do: flat at `readable` reads `crossings` **88 → 70** and `excess_len` −8.7%.

**§11.6's `label_near` was measuring a clearance under an attribution rule's name.** `tools/audit.py`'s class was "a label not nearer its own route than every other by a line of its own text", which was the right question against §11.9's strip grid, where a label's own gap ran from zero to four box heights and a reader had nothing constant to go by. Under §11.9.4's anchor that gap **is** the leader, exactly, for every label in the drawing — measured 2026-09-12, **0 of 192 placed boxes have a foreign leg strictly nearer than their own**. What the old form went on counting was a `leader + em` clearance, and it read 28. Restated as what is left worth counting — a stranger's line closer to the text than the text's own line is, so the reader has no gap to tell them apart by — it reads **10 of 203**. The class is now `a foreign line inside a label's leader`.

**And a router cliff at scale, which is not this section's.** `layout_tests.cpp`'s flat 2k target at `compact` now reads the straight router's numbers exactly, `through_box` 0 → 2,019: `ORTHO_VERTEX_BUDGET` bounds `nx * ny`, the reservation left the fold a squarer frame, and a squarer frame of 2k boxes spends more grid vertices than the same area laid out as a strip. Nothing on the corpus reaches it and `readable` at the same size does not. `[OWED]`: a budget whose shape is the router's reach rather than its bounding box.

**Area is the price and not the objection.** The goal set 2026-09-11 is zero violations before any search, and a larger picture is what correctness costs (§11.11). The guardrail is §11.12's comparison rather than area itself: at the 2026-09-07 reading scav is **5.94M pt²** against PlantUML's **13.78M** and elkjs's **6.54M**, so there is roughly 2.3× of headroom before growth costs the exit criterion it is measured by. `[OWED]`
...
**Decided 2026-09-09, superseding the strip grid above.** §11.9's placement enumerates five strips a box height apart on each side of each leg, so a label's distance from its own line varies from **0 to four box heights inside one drawing** — and at zero the line runs through the text. The distance is the one thing that tells a reader which line a label belongs to, and it is the thing the grid leaves free.

**The invariant: a label carries an anchor.** A fixed-length invisible segment runs from a point on the label's own polyline to one of **eight attachment points on the label's bounding rectangle** — its four corners and the midpoints of its four sides. Eight rather than four because a polyline is kinked: a label hung off a bend belongs on the diagonal, and a corner is what attaches there. The segment is never drawn; it is a placement constraint.

**Search parameters, and there are three.** Which point on the polyline the anchor sits at, which of the eight attachment points takes it, and nothing else — **the length is a constant**. The anchor slides along the whole polyline rather than per leg, in steps of half the label's height.

**The length is half an em, derived and not a knob.** `font_size_grid / 2`: **96 grid units at `readable`, 80 at `compact`**, so the tighter profile sits tighter for free and a font-size change cannot leave the leader looking wrong. Grid units are 1/16 pt and integer (§11.9's table), so half an em is representable at every profile the validator admits. Derived rather than a profile field because the profile is exactly 48 `int32` behind a `static_assert` and this is not a knob anybody tunes; if it ever becomes one, that is an ABI break and `profile_version` moves with it.

**What becomes structural.** **I2 is satisfied by construction** — the gap between a label and its own line *is* the leader length, exactly, for every label in every drawing (§11.9.3). The audit's detachment class stops needing a threshold and becomes an equality check, which is why the threshold question this section opened is closed by the model rather than by a number. And every label in one drawing sits the same distance from its line, so the reader learns that distance once instead of per label.

**What stays a feasibility test, and this is a limit rather than an oversight.** Eight attachment points at a free direction do **not** guarantee no slice: a label hung off a corner near a bend can still be crossed by another part of its own polyline. Non-slicing, and collision against state rects, text bands, settled boxes and foreign routes, remain tests over the candidate set exactly as above.

**The fallback inverts, and in the right direction.** Today a box with no feasible candidate keeps the centred placement, which breaks anchoring — the one property that ties a label to its transition — and the count of those is the count of sliced labels (§11.9.3). Under the anchored model the fallback **keeps the anchor and accepts a collision**: a label overlapping a box still reads as its own transition's, and a detached one does not. That is the trade the right way round, and it is why the fallback stops being a violation of the invariant this section is about.

**§11.3's charge gains a perpendicular half.** A label's width is charged to one rank boundary so the rank separation grows to hold it (§11.3, and §11.9.3 for the fold that was discarding it). An anchored label also needs room *beside* its leg: the corridor there must clear **`leader + box height`**, which nothing charges today. `[OWED]` with the model.

**Cost.** Candidates scale linearly with slide granularity, and halving the step doubles them. Placing 2k boxes is 66 ms against its 200 ms floor (§17 P7d), so the step is measured against that floor before it lands rather than chosen and hoped for. `[OWED]`
...
**Nothing here backtracks and nothing maximises globally.** Every dimension that is searched is searched greedily, strictly improving, and stops at the first local optimum. On 2026-09-19 this read "every stage is greedy and produces one answer", with Level 2's table as the only search there was; what the table says now:

| | chosen by | searched |
|---|---|---|
| which rank a state is in | longest path, then Level 1 rank moves (§11.10a) | greedily |
| which edge of a cycle is reversed | a depth-first walk in node order, then Level 1 reversals (§11.10d) | greedily — and `estop`'s best is out of greedy reach |
| whether a multi-rank edge is chained | always, then Level 1 cuts (§11.10b) | greedily |
| which face a route leaves and arrives by | the separation rule, then Level 1 face moves (§11.10e) | greedily |
| where in its rank it sits | median sweeps (§11.3) | no |
| its cross-axis coordinate | Brandes–Köpf (§11.7) | no |
| where a route runs | a grid search per net, in net order (§11.5) | no net is re-routed in light of another |
| where along a face a route is seated | five passes, per box and across two (§11.5, §11.10a) | no |
| the whole-chart knobs | `argmin(Cost, row)` over a fixed table | exhaustively, over the eight rows `portfolio_m` ships of the table's sixteen — so the fold bit, which is bit 3, is never varied |

**So `Cost` now shapes the drawing, along four dimensions, one move at a time.** It used to rank pre-declared drawings and never shape one, which is why "minimise bends" was true of the objective and false of the pipeline. What it cannot yet do is take a move that is worse on its own and better two moves later — which is exactly `estop`'s case (§11.10d) — or re-route one net in light of another.

**What *is* settled is the absolute layer.** Every Tier-0-shaped count is zero — `vanished` included since it became one (§11.6) — and so are label slices, unanchored boxes, attachments on a drawn corner, labels outside their enclosing state, arrowheads through a border, and labels over another route (§11.12). The corpus reads **27** reader-visible defects, of which 6 are lanes closer than a line of text and 8 segments flush along a box; it read 59, 37 of them crowded lanes, when this paragraph was first written. Crowding stopped being separation's problem alone once it was priced and the search could act on it.

**Level 2 and Level 1 are built; the three bounded moves of the original design and §11.6's surrogate are not.** Level 1 as built is not the per-frame strategy table below but four pin dimensions scored on the exact objective (§11.10a, §11.10b, §11.10d, §11.10e), and the rest of this paragraph is the design as it stood before that was measured. What the tree does today is P9b's, with P9c's compaction in bit 1 of its table: the fixed table of chart-global phase-2 tuples, the closed-form rule that says how many of its rows a chart runs, the `argmin(Cost, row)` reduction over what they produced, and the phase-1 hoist — each of them below, with what they measured in §17 P9b and §17 P9c. What is **[OWED]** is the per-frame level and everything under it, **gated at P9e** on a measurement taken there. `portfolio_m` is read by Level 2 and `sweep_count` by phase 1's crossing-minimisation sweeps (§11.3); `portfolio_k` has been read by Level 1 since 2026-09-19 (§0, §11.15). The order — **Level 2 first and Level 1 last** — is this section's, set 2026-09-08: every improvement measured to date is a whole-chart strategy choice rather than a fine-grained ordering move, so the level that chooses between whole charts was built first and the per-frame level and its moves are built only if a measurement says a reader can see what they reach. **That premise no longer holds and §11.10a replaces the conclusion drawn from it**: Level 2 is exhausted (71 defects against a floor of 70), in-rank order has no slack anywhere, and the dimension with the slack — which rank, and whether a run folds — is one this section ruled out. What P9a landed of all this is the two-level shape itself, written here before anything was built against it.

Local search from a structured seed with **restricted uphill moves** — "simulated sintering", Grover, DAC 1987 (~10% better *and* 3× faster than annealing from random). Not generate-and-test: random restart of a constructive heuristic measures ~2000× worse than guided search at equal CPU (Martí & Laguna), and published metaheuristic layout throughput is ~6–54 candidates/sec at 10–100 nodes. So what ships is a small fixed portfolio around the one constructive pipeline plus bounded moves off what it produced, never a candidate generator with a scorer bolted to its output. **The uphill half is not bought**: acceptance below is strictly improving, because an uphill rule needs a temperature schedule and this section has no measurement to set one from. It was a named lever wanted only if the moves were built and then stalled in a local minimum a reader can see. **Both conditions now hold**: on `estop` the reversal that leads to a drawing with one bend scores 3,218 alone against an incumbent of 3,073, so a strictly improving pass never takes it (§11.10d). A reversal is a structural choice and is better tried at the outer level than accepted uphill, so this case alone does not buy a temperature — but it is the first measured minimum the greedy pass cannot leave. `[OWED]`
...
**Phase 1's in-rank order is already optimal everywhere, so two of the three moves have nothing to search. Measured 2026-09-13, and this is P9e's gate answering.** The first move — swap adjacent in rank — was built as `transpose`, the exact half of the median heuristic §11.3 has only the approximate half of: for an adjacent pair the delta is the crossings drawn from those two nodes' own edges alone, `O(deg u + deg v)` and exact, because no other pair's contribution can change. It is correct — it never worsened a step on any chart — and it **improves nothing, anywhere**. Of 56 frames over the corpus and the element suite only 6 carry any crossings at all, transpose improves none of them, and on the 2k targets **1,098 frames carry zero between them**. The median sweeps reach a transposition-local optimum on every frame the tree has, and on almost all of them that optimum is zero.

**A loop-order finding worth keeping.** Run *inside* the sweep loop the same pass takes the corpus from 0 crossings to 7, while improving every individual step it makes: it moves the frame to a local optimum the next median sweep then starts from, so the trajectory changes and the best over it is worse. A greedy pass applied to the order the sweeps already kept cannot do that. Anything added to `minimize_crossings` belongs after the loop, not in it.
...
**The spread moves a net, not a seat, and the kinks between adjacent states were mostly not kinks.** Reviewed on the page 2026-09-19 as "kinks being introduced in transitions between horizontally adjacent states", and measured: of 105 adjacent pairs with a facing band, **59 have an inscribed glyph at one end** — a choice diamond or a pseudostate disc seats at one point per face, so two transitions at it cannot both be straight once they are separated at the other end, and that is structural rather than a defect. Of the 46 left, **38 are drawn as one straight segment and all 8 kinks have a genuinely blocked straight line.** There is no unnecessary kink between two adjacent boxes on this corpus.

What the review did find is that `ortho_spread_attachments` did not do what its own comment claims — "a pair seated apart at one box is seated the same way apart at the other and stays two straight lines". It moved the *seat*. The far end now takes the same displacement wherever its own face can seat it exactly, and keeps its seat where the face would clamp, which is the diamond case above. **It changes the drawing and no measure of it**: 146 straight routes, 705 segments, 401 bends, 8,164 ems of excess and 60 defects, all identical either way, with only the coordinate hash moving. Kept because it is a property the pass states, and pinned by two cases rather than by the corpus.

**A seat is held off its corner by the arc, not by the router's clearance. Landed 2026-09-13.** Found on `led`, where `Blinking -> Solid` runs 1,434 units and wobbles **29** of them a third of the way along. `onto_face` holds a seat off each face end by `max(clear, arc)`, and two reasons are folded into that number: a seat under a drawn arc points at canvas nothing was drawn on, and a seat at a face's very end leaves along the face it did not pick. The first wants the arc. The second wants to be strictly inside and nothing more — two *aligned* seats have collinear approach legs out of parallel faces, so neither has an adjacent face to be confused with. Spending 96 units of obstacle clearance on it cost alignment: the corpus held **13 transitions whose boxes overlap by exactly 163 units and whose seatable runs then miss by 29**, in five different charts, each paying a jog of 29 on a run of a thousand or more. `ortho_align_attachments` now insets by `max(arc, 1)` and no longer takes `clear` at all. Near-misses **16 → 4**, jogs shorter than the clearance **19 → 7**, `attachment on a drawn corner` still 0 of 514, corpus defects unchanged at 71 and no layout golden moved — this is a straighter line for the same picture.

#### 11.10a State placement is a search dimension, and it is the one that was missing

**Written 2026-09-13, after the measurements below contradicted this section's premise.** §11.10 says "every improvement measured to date is a whole-chart strategy choice rather than a fine-grained ordering move", and orders the levels from it. That is no longer true of the tree it describes, and the moves it went on to specify search the wrong thing.

**What the tree searches, and what decides a state's position.**

| decides | mechanism | searched |
|---|---|---|
| which rank a state is in | longest path after cycle breaking (§11.3) | **no** as written; greedily since the rank move below landed |
| where in its rank it sits | median sweeps (§11.3) | **no**, and there is nothing left to find — 1,154 frames over the corpus, the element suite and both 2k targets carry **zero** crossings between them |
| its cross-axis coordinate | Brandes–Köpf (§11.7) | no |
| whether its rank run folds | §11.4's two candidates, arbitrated by the scale measure | **locally, on aspect alone** |
| how the folded pieces pack | `pack_best`, and Level 2's `trybox` bit | yes |

**So "minimize total area" is specified as a score with almost nothing to score.** `w_area` and `w_aspect` are real and §11.6 weights them; the generator hands them eight chart-global tuples that cannot move a state to another rank. On `dock` all eight rows are **byte-identical**, so the area term is evaluated against one drawing. Reviewed on the page 2026-09-12: `dock`'s `Charging` sits a rank right of `Seated` and widens the `On` box past it, and `led`'s `Solid` sits a rank right of `Blinking` with the canvas half empty. Neither is reachable by any row.

**How it came to be ruled out.** §11.10 deletes *rotate subtree* because "ranks run in +x by construction and §11.7a's rank recovery from rects depends on that", and keeps two frame-local moves that both permute *within* a rank. Rank has been treated as topology, the way Sugiyama treats it. The deletion is right about rotation and wrong as a general principle: a rank *assignment* is a choice, and longest-path is one of several.

**Two mechanisms close it, and they are not the same size.**

**(a) The fold becomes a scored candidate.** It already produces two shapes per frame. Today `pack_better` picks between them on the scale measure — a local aspect ratio — and `Cost`, which owns area, never sees the alternative. Measured by forcing it: folding every frame reads **3,758M pt² against 4,680M, −19.7% of the canvas, for 7 more defects** (71 → 78). Neither end of that is what a fitted objective would pick, which is the point — the choice belongs to `Cost` and is currently made by something that cannot see area at all. The shape is a fourth row bit, `LAYOUT_SEARCH_ROWS` 8 → 16 and `portfolio_m`'s bound `[1, 8]` → `[1, 16]`, which is a `profile_version` bump. **This is the cheap one and it is where `dock` lives.** `[OWED]`

**The spine landed 2026-09-19, before any move that stands on it.** A move is expressed as an *input* to phase 1 rather than a mutation of its output: `order_submachines` takes `RankPin`s, each holding one state at a rank of the caller's choosing, and everything a frame derives from its ranks is re-run rather than patched. `rank_derived` is that tail extracted whole — the boundary charges, the chaining of multi-rank edges, the buckets and the crossing sweeps — and pulling it out moved no golden on either scale, which is the proof it is the same function.

**Three properties, each tested alone, because a search loop is only as sound as the thing it iterates.** A pin asking for the rank longest path already chose is the identity, so a move is measured against itself rather than against a re-derivation artefact. A pin that moves a state renumbers the ranks it emptied — phase 2 sizes a rank gap whether or not anything sits in it (§11.10), so `squeeze_ranks` runs with the pin. And **undoing a move is running with the pins one held before it**: there is no state to roll back, which is what makes an accept/reject loop possible without a transaction.
...
**The seam was the spacing-inflation retry, and closing it is one rule.** A drawing is a function of its tuple *and* its pins, and `layout_run` hands the pins back so a caller can re-derive it — but a candidate that inflated belongs to a profile the pins do not name, so no re-derivation could reach it. **A move that only fits once the whole chart's spacing was widened is not a better placement, it is a bigger drawing**, and refusing those closes the seam and states the right thing at once. `portfolio_k` ships at **24**.

**What shipping it reads.** Corpus **60 → 59** defects, element suite **16 → 15**, canvas **18.28M → 14.69M pt²** (−19.6%), **1.33× PlantUML → 1.07×**, aspect median 1.32 → **1.53** against a DAR of 1.60. Three element-suite pins moved and every one of them the right way: `fanin`'s refusals 2 → **0** — what the fan was refusing was room, and a move is how a frame gets room rather than asks for it; `regions`' doubling-back 3 → **2** at `compact`; and **`fork`'s capped branches 2 → 0 at both profiles**, which is the first thing in the tree to close §11.5's face-rule hole by choosing a better arrangement rather than by excusing the rule. One tripwire went the other way — a route flush along a box, 4 → 5 — and is raised with its reason rather than silenced.

**Two tests now say what they are about.** §11.10's argmin test and anything else re-deriving a row's geometry runs with `portfolio_k` at zero: with moves on, the geometry written is the row's *and* its pins', and a test that re-derives without them measures a layout nobody was shown.
...
**The rows contain no better drawing, and that is the finding rather than anything about the objective.** The per-chart floor — the fewest defects any row reaches, which is what a *perfect* objective would pick — reads **88 over the eight rows and 88 over the sixteen**. The eight new arrangements are not defect-free-er ones; they are the same drawings folded. `Cost` reached 89 against that floor both ways, so it is ranking as well as the table allows and the extra rows had nothing to offer it.

**Canvas is not the metric this turns on, and it should not be read as one.** A larger canvas is what satisfying more constraints costs (§11.11), so trading twenty percent of it for seven defects is a trade in the wrong direction whichever way the weights lean; what matters is that the fold rows move the floor by nothing. The canvas column above is there to say the rows *do* something, not to say which end is better.

**So the test §11.10a set is answered, and answered no — for a different reason than the section expected.** It asked whether a fitted `Cost` given both folds picks the drawing a reader picks. It does. The rows just do not contain a drawing a reader prefers. **(b), moving a node across ranks, is not built on the strength of this**, and what it now has to answer first is a representability question rather than a ranking one: does *any* rank assignment the corpus admits read fewer defects than the one longest-path produces? That is measurable without building the move — permute ranks directly and audit — and it is the measurement (b) should wait on.
...
**Level 2's rows run at once too.** A row is a whole independent layout of one chart, so the row loop parallelises exactly as the move sweep does and reduces in index order for the same reason. Row 0's diagnostics are the run's and every other row's are a candidate's business, so they are merged after the join rather than in the worker. Measured on the nested 2k target, no moves: 322 ms to 105 ms; on the flat 2k, 5,715 ms to 872 ms.
...
1. **Phase 2 is now the bottleneck and is the next incremental phase.** With routing reused, a warm candidate on `mill` is order 84 µs, size 224 µs, route 82 µs — phase 2 is 57% of it. The caching is the same shape as phase 3's but needs **two snapshots per frame**, not one: `size_sub` produces a frame's extent, and the owner's `size_state` then *grows* it to fill its packed row and rewrites the boundary nodes that sat on the old trailing edge. A clean frame under a dirty owner must be restored to the pre-growth snapshot so the owner's packing sees what it saw before; a clean owner restores the post-growth one over it. One snapshot gets this silently wrong.
2. **Then generate better candidates, not only more.** The sweep is blind — every (state, rank) pair and every chained segment. `Cost` already attributes: which routes carry the bends, which labels collide. Proposing moves against the worst-scoring frames first finds the same gain in fewer candidates, which multiplies everything above.
...
### 11.10d Which edge of a cycle is reversed

**Cycle-breaking chose it by walking in node order, which is declaration order.** A depth-first walk turns around whichever edge closes it, and that is an artefact of which `state` line was written first. It decides everything downstream: ranks, which edge then spans two of them, and therefore which one is chained and carries a bend waypoint. On `estop`, a three-state cycle behind an entry state, reversing `Tripped → Latched` instead of `Latched → Clear` leaves no edge spanning two ranks — nothing is chained and no waypoint exists — and Tier 2 falls **3,073 → 1,502** with bends **5 → 2**. Reordering the `state` lines in the source file alone does the same.

**`ReversePin { TransId, leg }` names the edge**, turned around before the walk so the walk finds that cycle already broken. It is a Level 1 dimension beside rank, cut and face, and `scav dump|render --reverse T:L` shows the drawing a reversal gives.

**It does not find `estop`'s answer by itself, and why is the finding.** The winning reversal scores **3,218 alone — worse than the 3,073 it would replace** — and a strictly improving greedy pass rejects it, although accepting it and continuing reaches 1,502. A structural choice that reshapes ranks, chaining and waypoints at once is not a local move. **So it belongs at the outer level, beside §11.10's tuple table**: try each reversal, run the rest of the search to convergence, keep the best. Built as §11.10f's perturbation.

One more thing this found: **`--portfolio-row N` silently disables Level 1.** Pinning a row makes the table one row long, and a one-row table is not scored, so the incumbent's cost is left at zero and no move can beat it. Per-row sweeps taken with it were measuring Level 2 alone. Fixed in §11.10f.

### 11.10e The face a transition leaves by

**The router picked it by which separation to the other end is larger** (§11.5), handed one point and one box, so it could not see the turn it was buying. On `estop`'s best arrangement `Tripped → Latched` left the left face and turned twice where the bottom face turns once. A better *rule* was built and measured over the corpus and is a loss (§11.5); the choice is small, discrete, and belongs to the search.

**`FacePin { TransId, leg, end, face }`** names the face — 0 left, 1 right, 2 top, 3 bottom, `end` 0 departure and 1 arrival. It reaches the router as `RouteNet::src_face`/`dst_face`, and `ortho_attach_face` honours it; the position along the face is still the other end's projection. Four per end is a Level 1 dimension, and `--face T:L:END:FACE` drives it by hand.

**One pin is declined rather than honoured: a face with no room to hold a seat.** A face shorter than twice the corner inset clamps every seat on it to one point, and a fork's bar, wide and thin, has two such faces; pinned, every branch stacked on one seat, and nothing in `Cost` priced it. A route that collapsed onto one point turned out not to be a pin's doing at all — five rank moves reached it — and is closed in Tier 0 (§11.6) rather than guarded here.

**What it bought.** On `estop` the face alone takes Tier 2 **3,073 → 2,477**; with the reversal pinned too, **990, and one bend in the whole drawing** — optimal for the graph. `gauntlet/regions` stops doubling back (**3 → 0**). And a sealed channel the inflation retry used to open by widening the whole chart three times is routed at its drawn size instead, by a face that does not cross the bar. **Over the corpus it made the drawings worse until crowding was priced**, which is §11.6's measurement and the reason that term exists.

### 11.10f The levels are composed by what they converge to

**Level 2 used to choose a row by its cost before Level 1 ran, and Level 1 then refined only that row.** The row that starts best is not the one that finishes best. Measured on `bottler`: at `portfolio_m` 8 the table picks row 0, which scores 154,348 before search and 51,281 after; at 16 it picks row 13, a start 19% better at 125,283, which Level 1 can only take to **77,441 — 51% worse**. Adding candidates made the drawing worse, which an `argmin` cannot do on its own and a greedy composition of two of them can.

**So every row is searched, and the rows are ranked by their converged cost.** Level 1 runs from each viable row of the table, and the pick is `argmin` over the results. Nothing can get worse by construction — the old pick's own converged result is among the candidates. Measured across the eight shipped rows before this was built: **corpus Tier 2 198,080 → 187,652, −5.3%**, with `bottler` −10.4%, `mill` −4.6%, `dock` −4.2% and `vac` −4.2%, and the other seven unchanged. The best converged row is the owner-hole ratio source (bit 2) on every chart that moved, and **no fold row (bits 3) is best on any of them**, which is why `portfolio_m` stays at 8.

**A cycle reversal is tried the same way, as a perturbation of the winner.** §11.10d's reversal scores worse on its own than the incumbent, so a strictly improving pass never takes it; this is iterated local search. After the winning row has converged, each edge that lies on a cycle in its frame and is not already the one reversed is tried as a start, searched to convergence, and kept when its *converged* cost beats the incumbent's. Rounds repeat until none does. Which segments lie on a cycle is phase 1's to say, from the strongly connected components of each frame's graph before it is made acyclic. Reversals also stay a Level 1 move: the first build took them out of Level 1 when it made them a kick, and `vac` came out **28% worse**, because the old pick's greedy result had used a reversal mid-search and was no longer among the candidates.

**As first built it was far too slow to ship, and three changes made it practical.** Searching all eight rows to convergence and cold-starting every kick took `mill` **654 s**, and warm kicks alone did not help it (1,124 s): a Level 1 round there scores about two thousand candidates, most of them eight faces per segment, so every extra search is expensive.

- **Rows are screened, then finished.** Every row gets a two-round search; the best two by screened cost — distinct in cost, since rows the compaction bit leaves identical rank as one — are searched to convergence, together with the row the unsearched table would have picked, so nothing is worse than before rows were searched at all. Only finished rows are ranked.
- **A kick searches only the frame it changed.** A reversal moves one frame and the others are already converged, so a kick starts from the incumbent's pins less that frame's rank, cut and face pins, and its Level 1 is offered moves in that frame alone. The best improving kick of each frame is combined and searched once, and kept when it beats the best of them alone; after the last round, one unscoped pass picks up whatever a frame's new size opened in its neighbours.
- **Label placement stopped scanning every obstacle for every candidate.** Profiling `mill` under real text put `place_labels` at **62% of all CPU**: a crowded chart finds no clear placement early, so the key prunes nothing and every one of a box's candidates — every half box height along every leg, times four leads and eight attachment points — tested every state, settled label and foreign route piece in its region. A uniform grid over those obstacles (`RectGrid`, `geom.h`) answers the same question, and the renders it produces are byte-identical on every chart compared. This is the third quadratic this project's search found in a phase that had been cheap enough to hide it (§11.10c).

`mill` under real text is now **1 min 45 s** at 714% CPU, against 7 min 51 s at 210% before the grid.

**What it reaches, read on the page first.** `tools/audit.py` under real text, against the commit before this section was built: **27 findings → 13**. Lanes closer than one line of text 6 → **2**; a segment flush along a box 8 → **4**; a label over a state box 5 → **2**; a foreign line inside a label's leader 4 → **0**; texts overprinting each other 3 → **2**; an arrowhead on no border 1 → 1. **One class is new: a label sliced by a region divider, 0 → 2.** The search found drawings its objective prefers where a transition label crosses a concurrency boundary, and nothing in `Cost` prices that. `[OWED]`: price it, or make it infeasible the way §11.9.4 treats a label over its own line. `estop` reaches the one-bend drawing §11.10e reached with the reversal pinned by hand, 990, with no pin.

**Tier 2 moved the same way, and that is a tripwire, not evidence** (§11.10). No space requests: **198,080 → 131,895, −33.4%**, with `mill` −44.4%, `brew` −30.7% and `toolchanger` −28.5%. Real text: **251,499 → 189,554, −24.6%**, with `estop` 2,477 → 990 and `led` −56.3%. **No chart is worse on either scale**, and `ota` alone did not move on either. The cut version comes out below the 137,776 that converging every row from cold reached, so screening gave up nothing that shows.

**And a kick reaches past a refusal.** The suite's 256-state ring with thirteen-rank skips is built so its frame is too large for the router, and at the unsearched order every one of its 510 transitions falls back to a straight line. With 24 moves per dimension, one reversal kick finds an order whose grid the router accepts, and **every transition routes**. The price is the chart's: each candidate on it takes about half a CPU-second, so those kicks' 2,304 candidates took 168 s. The case that tests what a refusal reports now runs the unsearched table.

**`[OWED]` The suites pay for the search now, and CI cannot yet.** One corpus pass at the shipped depth is about 100 s of wall clock on a 12-core laptop with no space requests and 180 s under real text. The unit suites lay each chart out once per binary and share it, which took the layout suite from 16 min to under 3. But `scav selftest` lays the corpus out at seven thread counts, one of them serial: 14 min 17 s a call here, and `test_selftest.py` calls it about a dozen times. On a four-core runner in a debug or sanitizer build that is past the job limit. Before this reaches a pull request, the matrix needs a selftest that checks thread counts on less than the whole corpus at full depth, or a candidate that costs less.

**A lone row is not screened.** A pinned row or a one-row table is its own finish, and screening it first only restarted it. The run's move count is now the pins it rests on beyond its seed rather than what its last search took: a finish starts from its screen's pins, and a kick drops the pins of the frame it re-decides, so a per-search count read zero for a run that moved.

**`--portfolio-row N` ran no Level 1 at all.** A pinned row made the table one row long, a one-row table was not scored, and so Level 1 started from an incumbent cost of zero that no move can beat — per-row measurements taken with it were measuring Level 2 alone. Every viable row is now scored, and Level 1 computes its incumbent's cost from its own start rather than being handed one.

### 11.10g What the page says the search cannot reach

**Read on 2026-09-25, eight charts, and almost none of the findings is a budget problem.** More search over the four dimensions Level 1 has fixes none of them. Each finding is one of four things, and which one decides the fix: a **construction rule** the phases should follow without being asked, a **search dimension** the search does not have, a **cost term** that prices the wrong thing, or a **phase bug**. The counterfactual below is how a finding is sorted.

**Two principles came out of the review, and they order everything after it.**

- **What is trivially right is construction, not search.** A straight line between two states is always preferred to a kinked one; a start state sits beside the state it enters; a route never runs along a state's border. These hold by construction in the phases. A multi-segment polyline is a concession to a layout's complexity, not a candidate the search happens to prefer. The search is for real tradeoffs.
- **Every rule gets an isolated test on a trivial layout.** A rule is proven on a chart small enough to name every route before its corpus effect is read, or a complex chart can never be trusted.

| chart | what a reader sees | class | cause |
|---|---|---|---|
| `brew` | `shot done` jogs once | phase | chained through a bend node in the middle rank, whose height the route followed; gone once the search reached a layout without the chain |
| `brew` | a T-shape with a quadrant of whitespace | search, and a dimension | row 0 searched to convergence reaches Tier 2 4,866 on a canvas 38% smaller than the 5,086 that shipped; §11.10f's screen dropped it. The layout a reader wants -- the main chain as a column beside `Brewing` -- needs ranks that run down, which no frame can |
| `brew` | the final state far right of `Standby` | construction | a final state is not seated beside its source, and the label's rank gap widens the distance |
| `dock` | the initial state far to the right | construction | reversing `* -> Empty` ranked the pseudostate after its target; and a pseudostate left-aligned in a wide column is far from its target whatever its rank |
| `dock` | `vacuum docked` wanders round `On`'s right edge | construction | `Empty` and `On` share a column and both are left-aligned in it; centred under `On`, both edges are straight verticals with their labels outside |
| `dock` | `On` and `lamp` ~20% too far apart | construction | a rank gap is `rank_sep` plus the label room plus the port-lane room, added, where the largest would do |
| `estop` | a label below its line, one far from its own line | label placement | §11.9 places labels as a phase, and nothing prices which side of a line a label sits on |
| `ota` | two edges between one pair wind and cross | phase bug | a rank pin left an edge within one rank, Brandes–Köpf aligned its two ends into one block, and the move that stacks `Writing` under `Fetching` drew one box over the other -- a Tier-0 layout, so the search never took it |
| `vac` | `Charging -> Solid` goes the long way round | construction | ports are placed per frame, not aligned with what they connect to |
| `vac` | `battery low` crosses an unrelated region and runs along `dock`'s border | phase bugs and a cost | a route in a frame could leave the state it runs inside through the region margin, and nothing priced crossing a region neither end is in |
| `vac` | overprinted text on `Blinking` | label placement | a label on a twenty-unit leg with nowhere to go |
| `regions` | routes between the two regions go round outside `Running` | phase (11.8) | the channel between two regions was routed in the frame `Running` sits in, where `Running` is an obstacle |
| `regions` | `lamp fault` leaves `light` by its far side | construction | a port's side is its in-frame edge's direction, not where the route goes |
| `regions` | `lamp fault` and `spun up` cannot both be direct | construction, then search | ports need one height across the divider and an order along it |
| `tcp` | `2MSL elapsed` runs the whole chart back | search dimension | a folded rank run always runs left to right; running the second piece the other way puts `TimeWait` under `Closed` |
| `axis` | `Homing`, `Parked` and the initial far right; `Moving` stretched | construction | a column is as wide as its widest member and every node starts at its left edge |
| `axis` | `Reached`'s two edges both kinked | construction | a diamond is met only at a face's midpoint, so both edges are straight only with its centre aligned with each neighbour, and the layering-axis position is not chosen |

**The counterfactual is how a finding is sorted.** `scav dump --layout` prints the row and every pin a drawing rests on as `rests on --portfolio-row R --rank S:R --cut T:L --reverse T:L --face T:L:E:F`, led by `--profile NAME` and `--no-text` where the run used them, and `--no-search` with those flags lays that drawing out again exactly. An edited copy scores the drawing a reader wants on the shipped objective: cheaper than what shipped means the search lacks a move, dearer means the objective is wrong.

**Decided:** every viable row is searched to convergence rather than the best two by screened cost (`brew` is the case against the cut); and the spacing-inflation retry stays. It lost its only fixture -- `sealed_chart` sealed because the router started a route outside the state it runs inside -- and `[OWED]`: a fixture that genuinely seals under the enclosure rule, until which its eleven cases are skipped with the reason.

**The plan, in order.**

1. **Built: the construction rules the review asked for, each with its isolated test.**
   - **Reversals only on a cycle.** A reversal is the choice of which edge carries a cycle's break, and a segment on no cycle has none; offering one ran `dock`'s initial arrow backwards.
   - **Brandes–Köpf gets only edges between consecutive layers, earlier end first.** A rank pin could leave an edge within one layer, whose two ends it then aligned into one block at one coordinate.
   - **Initial and final pseudostates sit `rank_sep` from the state they join and level with it**: an initial ranked just before its target and never moved by the search, a final mirrored after its source. A fold never cuts between one and the state it joins, and where the box it would move to is taken it stays where the layer put it. The corpus carries no initial arrow with a bend at either scale; `axis`, `bottler` and `toolchanger` did.
   - **Two Tier-0 terms**: `flush`, a route segment along a state's border, and `through_region`, a segment through a region neither end is in.
   - **A route stays inside the state it is drawn in.** The router keeps a band `max(1, min(clear, pad) / 2)` inside that state's border clear, a port on the border or an inner face within the band leaves square to it, a self-loop turns back before it, and nudging never lands a lane on a border. A route used to leave its state through the region margin, which is where `vac`'s run along `dock`'s border came from.
   - **The channel between two regions of one state is routed inside that state** (11.8), in the source region's frame.
   - **A port faces where its route goes.** After one sizing, a port whose far end lies on its frame's other side has its in-frame edge turned round, and the frames are laid out again; the pins a run reports include those turns, so re-deriving finds none left.
   - **Every viable row is searched to convergence**, and the rows ranked by what they reach.
   - **The counterfactual**: `rests on` and `--no-search`, above.
   - **The trace says what each rule decided**: `fold_cut` where a piece started or a cut was refused, `pseudostate_seated` moved, levelled or declined, `port_turned`.

   **What it measured, as tripwires** (11.10): Tier 2 with no space requests 131,895 → **118,787** (−9.9%), `toolchanger` +19.1% and `bottler` +2.2% the two worse; under real text 189,554 → **176,098** (−7.1%), `vac` +24.7% and `tcp` +12.7% the two worse. Every chart is at Tier 0 zero under the two new terms. The layout suite runs in under a minute with every row finished.
2. **Built: alignment in both axes, each rule with its isolated test.**
   - **Across: Brandes–Köpf aligns where a segment meets each end, not the two centres.** An edge carries an offset per end, the height of the port on a composite's border from the composite's centre, which is known because a state is sized before the frame it sits in; a block's members sit at those offsets from its root. A transition into a composite's lower inner state is one line through the port (`vac`'s `full`, `dock`'s `full`).
   - **Along: states joined by an edge inside one column share one centre line**, the widest one's, so both routes of a pair run straight down the overlap of their faces with a label outside each (`dock`'s `Empty` under `On`, `brew`'s and `ota`'s pairs). A group with two labelled edges widens its column to hold a label either side of the line; one label reserves nothing. Initial and final pseudostates are seated as before, and a bar keeps the column's leading edge, because a state stacked on a bar is met through its cap.
   - **A fold whose pieces pack back into one row, in their own order, is not taken.** It was the unfolded run at `node_sep` rather than `rank_sep` with every edge a cut crosses dropped from the alignment, and the scale measure preferred it by the rank gaps it removed.
   - **`flush` is a segment parallel to a border and nearer than the router's band**, not only one exactly on the line: `regions` at `compact` shipped a route one unit inside `Running`'s border and `bottler` one 32 units inside `Filling`'s, where a child and its parent leave only `pad` between them for a lane that needs clearance from both. Every corpus chart still reaches Tier 0 zero.
   - **`retrace`, a Tier-0 term**: a vertex where an axis-aligned segment turns straight back along the one before it. `regions` at `compact` drew a route down to a divider port and back up the same line.
   - **The trace**: `port_attached` for every segment met off a composite's centre and by how much, `column_centred` for every state moved along its column. `dump` takes `--profile` and `--no-text` like `render`, so the no-space scale the goldens are stated at can be dumped and traced, and a run's `rests on` names both.
   ...
   **What the page still shows, for the steps after this one:**
   ...
   - **Where the search lands moved.** `tcp` reached `TimeWait` under `Closed` part way through this step and does not now; `axis`'s `limit switch` and `held` share `stop`'s corner. Both are cheaper than they shipped before step 2, and neither is a rule this step owns.
3. **`[OWED]` Compaction along the layering axis**, a node as far toward its predecessors as the nodes it actually overlaps allow rather than past the widest member of the column before; and a rank gap sized to the largest of what it must hold, not their sum.
4. **`[OWED]` Port order along a border**, by the height of each port's far end, with a swap of two adjacent ports as the search's fallback.
5. **`[OWED]` Label rules**: above a horizontal line, near its own line, on the outer side of a parallel pair. With them, a parallel pair's spacing: the spread seats a pair `clear` apart, under the em that `crowding` and `label_near` both price, so every straight pair pays for its spacing. `crowd`'s pair costs 1,024 of its 1,904 this way, with `label_near` pinned at 346.
6. **`[OWED]` Frame orientation with top and bottom ports, and fold direction**, both search dimensions: ranks that run down (`brew`), and a fold piece that runs back (`tcp`).

**Blue sky, not planned:** a force-directed relaxation -- states repelling, transitions contracting -- run every so often over a candidate and rescored on the exact objective. A generator of candidates rather than a replacement for the scorer.
...
**The scored review is not a P7 gate, and it was never blind.** Decided 2026-09-05, with P7c and P7d landed and the three rendering defects a reviewer would have marked first — labels over names (§11.9), routes sharing a lane (§11.5), glyphs sized by text nobody draws (§11.4) — gone from the page. Two reasons. **Blinding is nominal**: PlantUML rounds its state corners and draws curves, elkjs draws sharp right-angled polylines in a horizontally biased layout, and scav is whichever panel is neither; hiding the labels hides nothing. **And a P7 render is a single candidate, not a layout.** §11.10's search — the portfolio, the local search, the calibrated weights — does not exist yet, so every scav panel is the first admissible layout the pipeline produced, adhering to its constraints and often plainly suboptimal within them. Scoring that against two engines that do optimise measures the missing optimizer, not the design, the way scoring P6 measured the missing router. So the comparison moves to P9's exit, where it is the test of what P9 built. **Four tools, and each answers a different question.** `tools/baseline.py` renders one chart three ways and `tools/poster.py` pages that up — that is the *comparison*, and its exit criterion is the incumbent's. `tools/candidates.py` renders and audits every row of §11.10's table and reports the per-class floor, the reachable best and what the pick costs against both — that is *headroom*, and it answers whether a defect belongs to the weights, to the search, or to a section, without the weights needing to be right first. `tools/review.py` puts every finding on the drawing it is about, badged and keyed with its class and its measurement — that is *the thresholds*, because three of the audit's classes rest on a number somebody chose and a count cannot say whether the call was right (§11.9.3). **The side-by-side itself stays the standing instrument**: the first two are re-run at every phase and the page is looked at, unblinded, for the defects a cost vector cannot see — that is how the compaction finding above was made and how P7c's shared lanes and P7d's labels were found before they were priced.
...
**Run 2026-09-19, and it is the reading P9d exits on.** Same instrument, all three engines rendered under real text, canvas normalised to points over the eleven charts:

| | canvas | vs scav | aspect (min–median–max) |
|---|---|---|---|
| scav | **18.28M pt²** | — | 0.72 · **1.32** · 2.55 |
| PlantUML | 13.78M | **1.33×** | 0.41 · 0.69 · 1.81 |
| elkjs | 6.54M | **2.80×** | 2.63 · 4.59 · 8.86 |

**Scav has gone from the smallest of the three to the largest, and the number that changed is its own.** PlantUML and elkjs read exactly what they read on 2026-09-07; scav went **5.94M → 18.28M**, 3.08×, which is §11.9.5's reservation and nothing else — the phases that grew it are the ones that reserve a lane per rank boundary and `leader + box height` beside each labelled leg. Per chart scav is still the smaller drawing on five of eleven (`brew`, `estop`, `ota`, `tcp`, `vac`), and the losses are the dense ones: `mill` 1.51×, `toolchanger` 1.52×, `bottler` 1.33×.

**What the canvas bought, on the same classes that reading listed.** Route segments 731 → **706**; a label over another route 8 → **0**; texts overprinting each other 5 → **3**; routes sharing a run 12 over 16,494 units → **6 over 9,637**; a segment flush along a box 3 → **3**; a label over a state box 5 → 6, which is now the collision §11.9.4's fallback accepts on purpose; every Tier-0-shaped count still zero. "A label nearer another route than its own" is not comparable — §11.10a restated it, and what replaced it reads 4 of 203. **Eleven classes the 2026-09-07 reading could not see are also zero**, including every label slice, every unanchored box and every attachment on a drawn corner.

**And the aspect is the finding the area column hides.** Scav's drawings span 0.72–2.55 against a stated DAR of 1.60; PlantUML's sit at a median of 0.69 and elkjs's at 4.59, both far off any ratio a page wants. On the one measure that is about the *shape* of a drawing rather than its size, scav is the only engine of the three whose output a reader could put on a page unrotated.

**So P9d exits met, with the area clause explicitly spent.** §11.11 set the goal as correctness first and recorded ~2.3× of headroom against PlantUML; that headroom is gone and 1.33× of the wrong side of it is spent. The trade is the one this phase was told to make and the counts above are what it bought, but **§11.12's own criterion is no longer a criterion scav passes**, and the next phase that touches sizing owns it. `[OWED]`: the canvas back, from search rather than by hand — §11.10a's fold rows already reach −19.7% and cost 7 defects, so the frontier exists and nothing on it has been chosen.
...
| cost | the ten Tier-2 weights, each with a ceiling keeping `Σ Tier-2` inside §11.2's budget. Each term is converted to the unit the profile names it in before its weight applies (§11.6) |
| search | `portfolio_k`, `portfolio_m`, `sweep_count`, `congestion_iterations`, `ripup_cap`, `spacing_inflation_cap` and `spacing_inflation_increment`. **Five of the seven are read today**: `sweep_count`, which phase 1 spends on crossing-minimisation sweeps (§11.3); `portfolio_m`, how many rows of §11.10's Level 2 table a chart runs — all of them whatever its size since 2026-09-20, shipping at 8 in `[1, 16]`; `portfolio_k`, the per-dimension cap on Level 1's candidates, shipping at 1,024 in `[0, 2^20]`; and the two spacing-inflation fields, which drive §11.6's retry. `congestion_iterations` and `ripup_cap` are validated and hashed and consumed by nothing, by the stages §11.5 and §11.9 hold as design. What follows is the field's history. **`portfolio_m`** landed at P9a in `[1, 64]` and shipped at 1, riding the `profile_version` bump the em already forced rather than paying for a second output-format change; **P9b is the change that pays** — 4 on both profiles, and the bound tightened to `[1, 8]`, the table's row count, since this section's discipline is reject-out-of-range rather than silently cap and a ninth row would repeat a tuple already run. **P9c takes the shipped value to 2** and leaves the bound where it is: bit 1 is compaction now, and rows 2 and 3 buy `dock` and `vac` a fifth and a twentieth of their Tier 2 at the price of twelve label collisions a reader sees, which is a trade only fitted weights can make (§11.6, §17 P9c). Landed in the same sub-phase: both profiles carry 2, and setting it moved the `inputs` column on every chart and no geometry anywhere — `dock` and `vac` hold P9b's picks, every other golden is byte-identical to the expansion commit's — with no second `profile_version` bump, 6 not having shipped. The cap inside the scaling rule stays as the belt for a profile that path never saw validated, the way `cost_of` floors the em (§11.6). No bound crosses the ABI: the extracted JSON and the generated ctypes carry a field's name, type, offset and size and never its range, so tightening one is not a binding change (§16). `sweep_count` bounds every fixed-count improvement loop, phase 1's sweeps included — one knob because they are one question, splittable at **P9d** if calibration wants different numbers |
| format | `print_columns` — the canonical printer's line-break budget (§15) |
| id | `profile_id`, `profile_version` — **10** on both shipped profiles, `w_crowding`'s arrival being the field that last moved it (it was 6 when `portfolio_m` moved it): a shipped number that moves geometry is a version (§17 P9b), and `portfolio_m` moves it again when P9c's shipped 2 lands (above) |
...
**A bend waypoint is a hard constraint, not a hint.** Recorded because the opposite was asserted here on 2026-09-19 and it was wrong. A net's anchors are `[src approach, waypoints…, dst approach]` and `route()` searches *leg by leg* between consecutive anchors, so the path reaches every waypoint. The reason a drawn polyline can appear not to touch one is `ortho_simplify`, which erases a doubling-back spur: on `estop`'s `Latched → Clear` the route goes out to the bend at x=2,823 and comes straight back, and the simplifier removes the spur while the turn it forced remains. So a waypointed net is aimed at its corridor on purpose, and §11.5's pair rule above excludes it for that reason rather than as a hedge.
...
*Exit, met, with one clause not met in the shape it was written:* **`vac`'s root frame occupancy is above 54.7%** — 69.98% over its four packed components, 60.24% over its five reader-visible rects — but the honest reading is that the 54.7% was row 0's canvas and **Level 2's pick at P9b is what moved it**, not either step here, both of which leave that packing exactly where they found it (§11.4). **Area is not down on every corpus chart, and the exception is every chart but one**: whitespace elimination cannot move an extent, so at the shipped M no chart's area moves on either scale, and at M = 4 area falls on `dock` alone, `vac` compacting at identical area — the clause was written for a step that turned out to be a knob. **Every element-suite property still holds** at both profiles, `crowd.scav`'s pinned `label` 4 and `label_near` 442 included, and the corpus's centred label fallbacks read 16 of 192 at the shipped M against 20 at M = 4 (§5). *Those four numbers are P9c's; P9d reads 5, 470 and 7 (§11.9.3).* **Rendered and looked at** (§11.12), and looking is what set the shipped M rather than any number in a golden.

**P9d — the objective un-blinded first, then the weights fitted, then the side-by-side that has waited since P6.** **Reordered 2026-09-09 on a measurement.** The page was read against the shipped pick of every corpus chart before any weight was touched, and it returned six absolute defects of which five neither `tools/audit.py` nor `Cost` could count (§11.9.3). Landed: a row of §11.10's table can be pinned so calibration can render a candidate the objective would never pick; the layout dump carries the cost vector so the number that ranked a candidate travels beside the defects a reader sees in it; `tools/candidates.py` renders and audits the whole table and reports the per-class floor and the pick's regret against it; five audit classes and one three-site exemption; the drawn corner no seat may sit on; and the placer and the fold that between them put a label on a state box. Reader-visible defects read **216 shipped, 207 in the best candidate the table holds, 178 as the per-class floor** — against 100/69/62 while the instrument was blind, and **193 of the 216 are labels**. `tools/review.py` puts every one of them on the drawing it is about, badged and keyed, because three of the classes rest on a threshold somebody chose. A weight fitted against an objective blind to four of its own violations would fit the wrong thing, so the fit begins after §11.9.3's owed work rather than before it.

**Then the weights.** The weights are fitted to `tools/audit.py`'s reader-visible counts — a label over a state or another route, a label nearer a stranger's line than its own, an overprinted string, an arrowhead over another route's end — over the candidate set P9b's Level 2 generates and **on the canvases P9c left**, which are the canvases it started with — whitespace elimination moved no extent and compaction is a row the shipped M of 2 does not run — so this is also where the shipped M goes back up if the fitted weights say rows 2–3 are worth running (§17 P9c). Then the unblinded page decides (§11.12): the counts choose which candidates are worth looking at, and looking is what settles it. **`crowd.scav` is the case that says the weights are wrong and that no search fixes it.** A label overlap costs 24 and an em² of canvas costs 1, so under the em the cramped drawing still wins — `t2` **1,230** shipped against **1,438** for the candidate whose every audit count is zero — and `brew` loses the same way. That is the weight fit, so it is this sub-phase: the pinned `label` **4 must go to 0**, and the carve-out in `functional_drawlist_tests.cpp` moves with it (§5). `sweep_count` splits here if calibration wants a different number for phase 1's sweeps than for search's (§11.15), and §11.3's global sifting and edge-weight schedule becomes judgeable for the first time.
