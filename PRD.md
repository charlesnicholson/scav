...
Reconciled against the tree on 2026-09-07. Phase names are §17's.

| Built and green | Owed, with an owner section |
|---|---|
| PB bootstrap, P0 language, P1 model spine, P2 loader, P3 printer and CLI, P4 space tables, P5a metrics + `DrawList` + reference builder, P5b SVG + baseline harness, P5c ABI JSON + Python binding, P6 real layout, P7a router boundary, P7b orthogonal router, P7c nudging, P7d label placement, inflation, and the bench, P7e the combinatorial stage, the attachment, and the element suite, P8 the thread shim, the sharded phases, the seeded structural hash, the determinism class, and `selftest` | **the incumbent comparison**, rescheduled to P9's exit because a single-candidate render measures the missing search (§11.12) · **routing between two concurrent submachines of one state** (§11.8), which no corpus chart exercises and the element suite pins · rip-up-and-reroute for the boxes no strip fits (§11.9) · **P9a–P9e search and calibration** (§11.10): P9a the scorer restructured and Tier 2 normalised · P9b the surrogate and the phase-1 hoist · P9c the two-level portfolio and the published latency form · P9d local search · P9e the fitted weights and the side-by-side · P10 viewer · P11 wasm · P12 editor |
...
**Three search fields are validated and read by nothing.** `portfolio_k`, `congestion_iterations` and `ripup_cap` are bounds-checked at load and hashed into the inputs digest (§6), so changing one moves a golden's inputs column without moving a coordinate. `portfolio_k` becomes live at P9c; the other two belong to stages §11.5 and §11.9 hold as design and nothing has scheduled. `sweep_count` and the two spacing-inflation fields are what the tree reads of that group (§11.15).
...
**The Tier-2 sum is an area measurement, and that is not a tuning problem** `[OWED, P9a]`. On the corpus totals `w_area * area` is **99.48%** of `t2` with no space requests and **99.38%** under real text. Every other weighted term rounds to nothing: bends 0.00% on both scales, corridor 0.14% and 0.05%, crossings 0.00%, excess 0.32% and 0.43%, aspect 0.05% and 0.04%, and `label_near` 0.09% on the one scale where it has a multiplicand at all. Over the whole of P7e `t2` moved **−0.35%** — 1.152747e9 → 1.148664e9 — while bends fell 15%, corridor 45%, crossings 12% and excess 43%, with aspect and area untouched. Those four terms were the entire change and the objective reported nothing. §19 and §17 P6 already hold that datum; what is new is the two paragraphs below it.

**It is untunable within the caps, because the terms are not the same degree in the coordinates.** Counts are degree 0 — bends, crossings, adjacency, `label`. Lengths are degree 1 — corridor, `excess_len`, `label_near`, and the aspect deviation. Area is degree 2. So the two ends of the vector sit 10⁷ to 10⁸ apart on real input: area 1.14e9 against 91 crossings with no space requests, 1.46e9 against 51 `label` under real text, and area against bends alone is 2.3×10⁶. One weighted sum over three degrees needs weights spanning that, and the ceiling above is 1024 — three orders of magnitude to cover seven. Raising the ceiling is not the fix; it is the only thing holding the sum inside `int64`.

**Determination: score every term in the unit the profile already names it in, so a weight is the exchange rate this section says it is.** A count is a count, a length is grid units and the profile states its separations in them, and an area is a product of two lengths — so normalising each term by its own unit before weighting leaves the weights as ratios between comparable quantities, which is the only reading under which "n means an edge would rather turn n times than cross" is a true sentence, and it is what makes the 1024 ceiling mean something. **Illustration, and explicitly not the unit P9a is to choose**: lengths in `node_sep` and area in `rank_sep × node_sep` put the corpus with no space requests at bends 497, corridor 120, crossings 91, excess 3,211, aspect 1,077 and area 6,888 — within two orders of one another — and re-score the whole of P7e at **−24.6%** instead of −0.35%. Two constraints bind whichever unit is chosen. **It must not quantise away a difference `tools/audit.py` can see**: floor-dividing `label_near` by `node_sep` produced a tie on `dock.scav` between two candidates the audit distinguishes. **And it must not divide by a separation a valid profile may set to zero** — `rank_sep`, `node_sep` and `sub_sep` are each `[0, COORD_MAX/4]` (§11.15). It is an output-format change like every other weight change: `profile_version` → 5 and every cost golden rebases, and the per-chart term-share table is committed as a golden beside them, so the next change to a weight is a number a reviewer watches move.
...
| | shipped | current-weight argmin | normalised argmin |
|---|---|---|---|
| bends | 426 | 410 | **393** |
...
**The exact scorer is the pipeline's most expensive stage, and the cheap form of it is exact** `[OWED, P9a]`. `cost_terms` takes **672 ms** on the nested 2k chart, against 28.5 ms for routing, 4.8 ms for sizing and 1.8 ms for ordering — the three of them being the whole of §17 P8's 35.3 ms `layout_run`, so the scorer costs nineteen times the layout it scores. On the flat 2k it is 54 ms against 124 ms of routing, and over the whole corpus 723 µs against 700 µs. **508–599 ms of the nested figure is the Tier-0 `through_box` loop alone**, which is O(pieces × live states × depth): sixteen million `ancestor_or_self` climbs, every one of them evaluated before any geometry test runs. Nothing has noticed because `layout_run` never scores — `cost_terms` is the caller's, and §11.10's search is the first caller that wants it per candidate.
...
**Determination: hierarchy descent.** Ancestry is flattened once per chart — `tin`/`tout` per state from one DFS, after which `ancestor_or_self` is two comparisons, 14–22 µs at 2k — and one uniform grid is built per submachine over its live children, siblings being disjoint by Tier 0, at 25–33 µs. Tier 0 then descends from the root submachines' children and prunes a subtree the moment a piece does not `enter` its box: **exact under child ⊆ parent alone**, assuming nothing of §11.5 or §11.8, and it catches `regions.scav`. `corridor` buckets by `(axis, coordinate)`, a shared run implying collinearity; `crossings` takes, per vertical, the band of horizontals whose y lies strictly inside its span, with a brute-force fallback for the degraded diagonal pieces; and `box_overlap` goes through the same grid — it is O(Σ kids²) and is what is left of the flat 2k, 2.04 ms of 2.55. Every sum stays order-independent, so §6 is untouched, and nothing new lands in `Routes`, so `cost_columns` keeps scoring another build's output. **Measured: nested 2k 672.3 → 2.89 ms, flat 2k 51.2 → 2.55 ms, corpus 11.21 → 1.89 ms per ten scorings, with identical `CostTerms` on all 88 runs** — `straight`'s nonzero `through_box` included.

**Cross-frame Tier-2 pairs exist on the shipped router, so this is not only a Tier-0 argument.** One crossing and 1,312 corridor units on `readable` — `bottler`'s `valve`↔`cell` and `tcp`'s `inbound`↔`outbound` — every one of them tracing to §11.8's hole, so a per-frame sweep would silently move numbers this document publishes. **`label` and `label_near` are what this leaves unbounded**, at O(placed × states) + O(placed × pieces), and they are unmeasured because the phase tables have no placed boxes to score. P9a measures them under real text.

Area is deliberately last: minimizing it directly produces crammed blobs with snaking edges, and narrow channels force bends. **That is true of the multiplier and false of the influence** — `w_area` is 1, the smallest number in the list, and `w_area * area` is 99% of the sum (above), so the ordering states an intent the arithmetic inverts. For the packing sub-problem the objective is the **scale measure** `SM = min(DAR/w, 1/h)` — held as a rational and **compared by cross-multiplication, never computed**. Its tiebreak order (area, then aspect) is a versioned profile field.

**The surrogate is one object and this section described two** `[OWED, P9b]`. "Crossings from straight-line segments" is geometric; "inversion counting in the layered formulation" is combinatorial, and valid only for a pair of edges sharing one rank gap in one frame. A search cannot have both under one name, and what a search needs is the first. **The surrogate is specified against `SubmachineOrders` and `SizedLayout` and no `Routes`.** Phase 3's planning walk — `route.cpp`'s net planning, which turns a transition into its segments' ends and bend waypoints — is extracted into a `plan_nets()` that both the surrogate and `route_transitions` call, so the surrogate's ports are the router's rather than a second guess at them. Every end is then seated with the router's own exposed primitives, each already pure: `ortho_attach_box`, `ortho_reface_attachments`, `ortho_align_attachments`, `ortho_spread_attachments`, `ortho_clearance`. Each net is then one straight chord, seat → bend waypoints → seat. Whole-chart complexity `O(|E| log|V| + |E| log|B|)`.
...
**[OWED in full: nothing here is built.]** `portfolio_k` is validated and read by nothing (§0, §11.15), and `sweep_count` is read by phase 1's crossing-minimisation sweeps (§11.3) and by nothing else. Everything below is design owned by this section and scheduled as §17's P9a–P9e.

Local search from a structured seed with **restricted uphill moves** — "simulated sintering", Grover, DAC 1987 (~10% better *and* 3× faster than annealing from random). Not generate-and-test: random restart of a constructive heuristic measures ~2000× worse than guided search at equal CPU (Martí & Laguna), and published metaheuristic layout throughput is ~6–54 candidates/sec at 10–100 nodes. So what ships is a small fixed portfolio around the one constructive pipeline plus bounded moves off what it produced, never a candidate generator with a scorer bolted to its output. **The uphill half is not bought**: acceptance below is strictly improving, because an uphill rule needs a temperature schedule and this section has no measurement to set one from. It is a named lever for P9e, on the evidence that the search stalls in a local minimum a reader can see.
...
- **Level 1 — per frame, on the surrogate (§11.6).** A frame evaluates its own K strategy rows inside its own shard body into a K-slot array and takes `argmin(surrogate, k)` in index order, never against a shared best-so-far (§6). Every candidate gets its own `Frame` and `SegPort` vectors, and the per-shard scratch resets after every *candidate* rather than after the frame — a stale frame-local index read by the next frame in the same shard range would make output a function of shard count.
- **Level 2 — whole chart, on exact `Cost` with real routing.** M tuples of the chart-global phase-2 knobs: the packer choice (`trybox`), the scale-measure tiebreak, and DAR handed down to each frame from its owner's remaining hole instead of the global `dar_num`/`dar_den` — today `pack_better` measures every frame at every depth against 16:10, which is `vac`'s 55% occupancy one level down (§11.4).

**Level 2 is affordable, and it is where the real-text wins are.** Exact scoring is 3 ms at 2k after §11.6's restructuring and routing is 0.7 ms over the whole corpus, so a handful of exact evaluations cost about what one used to. It has to be the exact scorer: the surrogate cannot see `corridor`, `label` or `label_near` at all, and those are the terms a reader reads. Over the 28-point grid of knobs that already exist, under real text, the normalised argmin takes label overlaps **51 → 13**, `corridor` **16,494 → 7,184** and excess **1,583,984 → 1,197,094** over the corpus at **+10.8% area** — which is the trade P9e's weights have to price, not a free win.

**What search runs inside, informational** (one thread, `readable`, no space requests, this machine): the nested 2k `layout_run` is 35.3 ms — order 1.8, size 4.8, route 28.5 — and the flat 2k routes in 124 ms; over the whole corpus order is 192 µs, size 779 µs, route 700 µs. `cost_terms` was **672 ms** on the nested 2k before §11.6's restructuring and **2.89 ms** after, which is what makes level 2 a design rather than a wish. §5 keeps a clock out of a gate, so these are reported; what P9c publishes is the closed form below rather than these numbers.

**The K strategies are the first K rows of a fixed, versioned table, and nothing is seeded.** A row is a phase-1 knob tuple: sweep count (0 included), sweep start direction (predecessors or successors), the pull-right ranking pass on or off, and initial in-rank order (document order or DFS discovery). Changing the table bumps `profile_version`, and **no search constant lives outside the profile or that table** — the inputs digest hashes all 47 profile fields (§6), so a constant that is neither is one no golden can see. **"Seeded by submachine id" is dropped**: `SubmachineId` is an ordinal, so a re-parse after inserting one state renumbers every later submachine, every later frame then draws a different seed, and the diagram cascades — against §11.11 and against P9's own exit clause about a one-state edit. If randomness is ever wanted it is `rnd(seed = the model's structural digest, phase = a constant per kind, item = submachine ordinal, step = candidate index)`, seeded from the model and never from a build constant (§6).

**How much search a frame gets is closed form and model-derived — never a wall clock, never a thread count.** With n the frame's ordering-node count after chaining (§11.3), `k_frame = max(1, portfolio_k >> max(0, ilog2(n) - 6))` and `sweeps_frame` the same, so a frame of ≤64 nodes gets the full K and the flat 2k frame gets one candidate; M derives from `entity_count` the same way, so the flat 2k chart runs M = 1. **Said plainly: the largest frame gets the least search.** That is backwards for quality and right for latency, and it is the honest form of §11.11's bet — the shape that cannot be searched fast is searched least, rather than given a second mode. Timing it instead would put one chart at two costs on two machines.

**Three moves survive.** Frame-local: **swap adjacent in rank**, and **move a node across ranks**, the second of which can flip §11.4's fold, so its frame is re-sized. Chart-level and serial: **reorder sibling submachines**, subject the owner state's ordinal, parameter an adjacent-transposition index in the submachine span order phase 2 feeds the packer, scored exact after phase 2 — the move §11.8 has wanted since `w_adjacency` was written. Per sweep:
...
**Phase 1 and its search run once, outside the retry loop.** Phase 1 reads exactly one profile field (`sweep_count`) and no extent, so it is separation-invariant and `layout_run` today recomputes an identical `SubmachineOrders` on every spacing-inflation attempt (§11.6). Chart-level moves come after phase 2 and so live inside an attempt: they are **replayed on each inflated attempt, not re-searched**, which keeps "the attempt that degraded least" a comparison between attempts of one topology instead of between two different diagrams.
...
| search | `portfolio_k`, `sweep_count`, `congestion_iterations`, `ripup_cap`, `spacing_inflation_cap` and `spacing_inflation_increment`. **Three of the six are read today**: `sweep_count`, which phase 1 spends on crossing-minimisation sweeps (§11.3), and the two spacing-inflation fields, which drive §11.6's retry. `portfolio_k`, `congestion_iterations` and `ripup_cap` are validated and hashed and consumed by nothing until §11.10's portfolio (P9c) and the stages §11.5 and §11.9 hold as design. A **`portfolio_m`** field, for §11.10's chart-level portfolio, is **[OWED]**: P9c reads it, and it lands at P9a, riding the `profile_version` bump the normalised weights already force rather than paying for a second output-format change. `sweep_count` bounds every fixed-count improvement loop, phase 1's sweeps included — one knob because they are one question, splittable at **P9e** if calibration wants different numbers |
...
**P9a — the docs commit, the scorer, and the weights.** The phase opens with documentation and no code, because §11.10 as first written specified a `flip port side` move over a side nothing chooses, a `rotate subtree` move over an axis §11.7a's rank recovery depends on, a portfolio seeded on `SubmachineId` — an ordinal that renumbers under exactly the one-state edit P9's own exit clause is about — and a per-submachine `argmin(Cost)` for a cost with no per-frame form. Two of the five moves, the per-submachine `argmin(Cost)` and the never-re-score rule are struck and the two-level design written in their place before anything is built against them (§11.10, §11.6, §0). Then the code: `cost_terms` is restructured off the work that makes it 672 ms on the nested 2k, and Tier 2 gains §11.6's normalisation, because the shipped weights make the Tier-2 sum an area measurement to within a rounding error (§19) and calibration cannot start from a scalar one term owns. `portfolio_m` (§11.15) lands here too, riding the `profile_version` bump the weights already force rather than paying for a second output-format change. And the shape the normalisation is about gets a gauntlet chart (§5): `brew`'s two parallel labelled transitions into a packed composite, where area and legibility pull opposite ways.
*Exit:* the nested 2k chart scores in **≤3 ms**, with `CostTerms` identical to the old scorer on all **88** chart × profile × router runs — a restructuring, not a re-specification — and the per-term share table committed as a golden; P6 through P7e re-scored under the new weights, where an illustrative normalisation reads the span as **−24.6%** of Tier 2 against the old weights' **−0.35%** (no space requests, the scale these tables use).

**P9b — the surrogate, its ranking test, and the phase-1 hoist.** §11.6's surrogate is built with its ranking property test: a misranking surrogate optimises the wrong thing silently, so that test is what makes the whole of level 1 trustworthy, and its performance floors are pinned so it cannot drift into a second exact scorer. One strict win rides with it: phase 1 is hoisted out of the spacing-inflation retry loop — it reads one profile field and no extent, so every attempt today recomputes an identical `SubmachineOrders` (§11.10). The fold's second layout is not the same kind of win: §11.4 measured that the cut fires on 59 of the corpus's 62 components, so the folded candidate is a different shape almost everywhere and skipping it where it is not saves nothing; what phase 2 can save across candidates is P9c's caching (§11.4).
*Exit:* the ranking property test green with its floors pinned; phase 1 hoisted, with every geometry column byte-identical on the corpus and both 2k shapes at every thread count, since the hoist may not move a coordinate.

**P9c — the portfolio, both levels.** Level 1 evaluates each frame's `k_frame` strategy rows on the surrogate inside that frame's own shard body, the rows coming from §11.10's fixed versioned table and from no seed; level 2 evaluates `portfolio_m` chart-global phase-2 tuples on exact `Cost` with real routing. The scaling rule is closed form and model-derived — `k_frame` and `sweeps_frame` shifted down by `ilog2(n) - 6`, M off `entity_count` — so the flat 2k chart runs one candidate at each level, and 2k latency is a published formula rather than a measurement somebody repeats per machine. There is a candidate to beat before any of it is built: under real text `axis` ships one that a point of the existing 28-knob grid dominates on every term but crossings.
*Exit:* `axis`'s dominated shipped candidate gone; the determinism class and `selftest` green at {1,2,3,5,8,13,16} with the per-frame chosen-strategy arrays asserted **byte-identical rather than hashed**, a hash saying that something moved and localising nothing; a third 2k fixture of near-tied symmetric frames, so the tie-break rules are what decides them and not the input's asymmetry; and **full-layout latency at 2k measured and published as the closed form** (§11.11's bet), the first of P9's original three clauses.

**P9d — local search.** The three surviving moves (§11.10): swap adjacent in rank and move a node across ranks, frame-local, and reorder sibling submachines, chart-level and serial. A sweep enumerates into a dense prefix-summed index with every slot written, evaluates the deltas over `layout_shard_count(c)` shards with no reduction, accepts in one serial index-ordered pass under the disjoint-subject rule, and re-runs the empty-rank squeeze at the end of it. The inner loop delta-evaluates inversions on `pos` and the frame re-sizes once per accepted sweep, which is what holds a sweep affordable on the frame sizes level 1 gives full K to.
*Exit:* property tests over the move neighbourhood — every enumerated slot feasible or sentinel, every accepted move's delta reproduced by a full re-score, no rank left empty and no two ranks left at one x; determinism green at every thread count; and **two named mutations must fail**, acceptance in shard order and a shared best-so-far, those being the two ways this stage stops being a function of the model.

**P9e — calibration, and the side-by-side that has waited since P6.** The weights are fitted to `tools/audit.py`'s reader-visible counts over the candidate set the earlier sub-phases generate — a label over a state or another route, a label nearer a stranger's line than its own, an overprinted string, an arrowhead over another route's end — and then judged on the unblinded side-by-side (§11.12), in that order: the counts choose which candidates are worth looking at and the page decides. `sweep_count` splits here if calibration wants a different number for phase 1's sweeps than for search's (§11.15). Two levers written to be bought when review says the term is wrong become judgeable for the first time, §11.4's compaction and whitespace elimination and §11.3's global sifting and edge-weight schedule, and the renders already say which is first: compaction, because the visible price the legible candidates pay is whitespace.
