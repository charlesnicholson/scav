...
**Two more, counted but mis-tiered.** Routes sharing a lane is `corridor` at `w_corridor = 48`; the reviewed rule is narrower than the term — sharing is forbidden **except** for transitions with a common *destination*, where the last kink may be shared, and even then it should be weighted low, since it pays only on dense diagrams. And two lanes may not sit closer than the type they carry: `ortho_clearance` is `node_sep / 3`, **96 units at `readable` against a 160-unit text height**, so a legal pair of lanes is narrower than the glyphs beside them. `[OWED]`

**A seat may not sit on a drawn corner, and one site still does.** Landed: `state_corner_radius` is the one statement of the arc, capped at the interior ring — it was `min(w, h) / 8`, a proportion with no bound, so a 12,904-unit composite drew a 1,613-unit arc against a 128-unit ring, which is also why an initial state placed at the ring sits outside the shape. Corner attachments went 13 of 257 to 6, worst case 94% of the radius to 25%. **The six remaining are boundary nodes**, which `size.cpp` places at a frame edge and no router seats, so they need the same rule at that site (§11.5). `[OWED]`
