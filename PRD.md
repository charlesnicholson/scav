...
1. **Consolidate and commit** what step 1 built: the counterfactual; reversals only on a cycle (a reversal is the choice of which edge carries a cycle's break, and a segment on no cycle has none); the Brandes–Köpf fix (an edge is handed to it only between consecutive layers, earlier end first); initial pseudostates ranked just before their target, flush against it and never moved by the search, and final ones mirrored after their source, both level with the state they join; two Tier-0 terms, a segment along a state's border and a segment through a region neither end is in; routes kept inside the state they are drawn in, off its border, a port on it leaving square; the channel between two regions routed inside their state; ports turned to face where their route goes; every viable row finished. Each with its isolated test, the goldens rebased with reasons.
...
5. **`[OWED]` Label rules**: above a horizontal line, near its own line, on the outer side of a parallel pair.
