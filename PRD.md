...
**The developer-facing override is `SCAV_ENVY_CACHE_ROOT`, not `ENVY_CACHE_ROOT`.** envy's own variable is global, so a shell-profile export retargets every envy project on the machine — including ones that chose a project-local sandbox deliberately, which is the same decision scav made. A scav-scoped name is safe to leave in a profile; `build.sh`, `build.bat` and `tools/build.py` translate it to the variable envy reads. Precedence is `ENVY_CACHE_ROOT` > `SCAV_ENVY_CACHE_ROOT` > manifest directive, and the build prints which tier answered.
