...
**The sandbox is the promise; the override is a power-user opt-in.** Someone who builds scav once and does not care about envy should be able to `rm -rf out` and have nothing left on their machine, so the manifest default is not negotiable and is checked by a test that runs whether or not envy is required. What the override buys is one shared toolchain copy across worktrees, which matters to a maintainer with several and to nobody else.

**It is spelled `SCAV_ENVY_CACHE_ROOT`, not `ENVY_CACHE_ROOT`.** envy's own variable is global, so leaving it in a shell profile retargets every envy project on the machine — including ones that chose a project-local sandbox deliberately, which is the same decision scav made. `build.sh`, `build.bat` and `tools/build.py` translate the scoped name to the one envy reads, and each also reads it from a gitignored `.env` so it can be written once: Conductor copies `.env*` into every new workspace, so a fresh worktree inherits it. The file is parsed, never sourced — a line in it must not be able to run as the developer. Precedence is `ENVY_CACHE_ROOT` > `SCAV_ENVY_CACHE_ROOT` > `.env` > manifest directive, and the build prints which tier answered.

**Not a CMake preset**, which is the obvious place to look. envy resolves the cache and hands back absolute paths to cmake, ninja and python before CMake is invoked at all, so a preset is read several envy calls too late to influence it.

**Why translation rather than a manifest directive.** envy's cache directive does understand `${VAR:-default}` — verified — but only in the envy *binary*. The shell launcher that bootstraps that binary has a simpler expander that handles `$VAR` and `${VAR}` only, so it would take `${SCAV_ENVY_CACHE_ROOT:-out/.envy}` literally and bootstrap into a directory named that. `ENVY_CACHE_ROOT` is the first tier in both resolvers, so translating to it is the one spelling they cannot disagree about. Accepted consequence: a `bin/` shim run directly, outside `build.sh`, still resolves the manifest default. If envy's launcher gains `:-` support, the directive becomes the tidier home for this.

**Sharing one cache is safe, and measured rather than assumed.** Packages are keyed by content fingerprint, so an override is usually a partial hit on arrival — scav's `envy.cmake@r0` is the same build another project already fetched. Three simultaneous `envy sync` runs against one fresh cache all succeed: one installs, the others hit, and the resulting tree is correct. So parallel agents in separate worktrees can share one cache without a lock of our own.
